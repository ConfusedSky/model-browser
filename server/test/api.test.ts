...
describe("same-origin guard", () => {
  ...
  it("and the bytes it does serve are unchanged by that", async () => {
    ...
    const direct = Buffer.from(
      await (await get("/api/file?path=/loose.stl")).arrayBuffer(),
    );
...
describe("GET /api/file serves models, and only models", () => {
  ...
  it("refuses one on the launch path too, spawning nothing", async () => {
    ...
    const launching = createApp(
      ...
      createLauncher({
        env: {
          HOME: join(fx.dir, "no-such-home"),
          XDG_DATA_DIRS: join(fx.dir, "no-such-share"),
        },
        exec,
        config: {},
      }),
...
describe("GET /api/file byte ranges", () => {
  ...
  it("answers each of the three spellings with the slice it names", async () => {
    ...
    for (const [header, start, end] of cases) {
      ...
      expect([header, res.headers.get("content-length")]).toEqual([
        header,
        String(end - start + 1),
      ]);
      expect([header, res.headers.get("accept-ranges")]).toEqual([
        header,
        "bytes",
      ]);
      ...
      expect([
        header,
        (await bytes(res)).equals(whole.subarray(start, end + 1)),
      ]).toEqual([header, true]);
    }
  });
  ...
  it("serves the whole file for anything else, and every 200 says ranges are welcome", async () => {
    ...
    for (const header of [
      ...
      expect([header, (await bytes(res)).equals(whole)]).toEqual([
        header,
        true,
      ]);
    }
  ...
  it("answers every range against a zero-length model with 416, not a stream error", async () => {
    ...
    try {
      for (const header of ["bytes=-5", "bytes=0-"]) {
        ...
        expect([header, (await res.arrayBuffer()).byteLength]).toEqual([
          header,
          0,
        ]);
      }
      ...
      const whole = await emptyApp.request("/api/file?path=/empty.stl", {
        headers: LOOPBACK,
      });
...
describe("GET /api/dir", () => {
  ...
  it("echoes one spelling of a path, whatever spelling was asked for", async () => {
    ...
    for (const spelling of ["//sub", "/sub/.", "/sub/", "/loose.stl/../sub"]) {
      ...
      expect([spelling, ((await res.json()) as DirListing).path]).toEqual([
        spelling,
        "/sub",
      ]);
    }
  ...
  it("refuses an entry half on something that is not an archive, without an errno", async () => {
    ...
    for (const path of ["/sub!/", "/sub!/x", "/notes.txt!/", "/notes.txt!/x"]) {
      for (const extra of ["", "&flat=true"]) {
        const res = await get(
          `/api/dir?path=${encodeURIComponent(path)}${extra}`,
        );
...
describe("zip virtual folders", () => {
  ...
  it("a directory named *.zip inside a zip stays navigable", async () => {
    const res = await get(
      `/api/dir?path=${encodeURIComponent("/models.zip!/v2.zip")}`,
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as DirListing;
  ...
  it("lists a folder inside a zip", async () => {
    const res = await get(
      `/api/dir?path=${encodeURIComponent("/models.zip!/parts")}`,
    );
    const body = (await res.json()) as DirListing;
  ...
  it("rejects entering a nested zip with a clear message", async () => {
    const res = await get(
      `/api/dir?path=${encodeURIComponent("/models.zip!/inner.zip")}`,
    );
    expect(res.status).toBe(400);
    const body = (await res.json()) as { error: string };
  ...
  it("decompresses a single entry on demand with intact bytes", async () => {
    const res = await get(
      `/api/file?path=${encodeURIComponent("/models.zip!/parts/lid.stl")}`,
    );
    expect(res.status).toBe(200);
...
describe("GET /api/complete", () => {
  ...
  it("returns empty for unreadable parents", async () => {
    const res = await get(
      `/api/complete?prefix=${encodeURIComponent("/nope/nothing/here")}`,
    );
...
describe("thumbnail cache API", () => {
  ...
  it("miss before any put", async () => {
    const res = await get(
      `/api/thumb?path=${encodeURIComponent(path)}&mtime=111`,
    );
  ...
  it("carries a discard over the wire — null clears, and is not a bad axis", async () => {
    ...
    const res = await get(
      `/api/thumb?path=${encodeURIComponent(other)}&mtime=111`,
    );
    const body = (await res.json()) as ThumbGetResponse;
    expect(body.status).toBe("hit");
  ...
  it("keys one file on one entry, however the path was spelled", async () => {
    ...
    expect(put.status).toBe(200);
    const res = await get(
      `/api/thumb?path=${encodeURIComponent("/loose.stl")}&mtime=424`,
    );
    const body = (await res.json()) as ThumbGetResponse;
    expect(body.status).toBe("hit");
    expect(body.png).toBe(png);
  ...
  describe("the occlusion dimension", () => {
    ...
    it("reads an entry written without `ao` as the occluded render, under `ao` absent and `ao=on`", async () => {
      ...
      for (const q of ["", "&ao=on"]) {
        const body = (await (
          await get(`/api/thumb?path=${encodeURIComponent(noAo)}&mtime=111${q}`)
        ).json()) as ThumbGetResponse;
        expect(body.status).toBe("hit");
        expect(body.png).toBe(png);
    ...
    it("reads its unoccluded render as stale, carrying the shared camera and axis", async () => {
      const res = await get(
        `/api/thumb?path=${encodeURIComponent(noAo)}&mtime=111&ao=off`,
      );
      const body = (await res.json()) as ThumbGetResponse;
    ...
    it("reads the unoccluded render of an entry with no orientation as a miss", async () => {
      ...
      const body = (await (
        await get(
          `/api/thumb?path=${encodeURIComponent(bare)}&mtime=111&ao=off`,
        )
      ).json()) as ThumbGetResponse;
    ...
    it("writes the sibling render on `ao:false` and leaves the occluded labels alone", async () => {
      ...
      const on = (await (
        await get(`/api/thumb?path=${encodeURIComponent(noAo)}&mtime=111&ao=on`)
      ).json()) as ThumbGetResponse;
    ...
    it("rejects an `ao` query that is neither on nor off", async () => {
      const res = await get(
        `/api/thumb?path=${encodeURIComponent(noAo)}&mtime=111&ao=maybe`,
      );
      expect(res.status).toBe(400);
...
describe("thumbnail cacheability", () => {
  ...
  it("never lets a miss be stored — the tile that would stay empty forever", async () => {
    const res = await get2(
      `path=${encodeURIComponent("/nothing-here.stl")}&mtime=1`,
    );
    const body = (await res.json()) as ThumbGetResponse;
    expect(body.status).toBe("miss");
  ...
  it("answers a PUT with the generation it wrote, and the next read agrees", async () => {
    ...
    const body = (await (
      await get2(`path=${encodeURIComponent(path2)}&mtime=111`)
    ).json()) as ThumbGetResponse;
    expect(body.status).toBe("hit");
  ...
  it("pins a read that names the current generation, and only that read", async () => {
    ...
    const fresh = await get2(
      `path=${encodeURIComponent(path2)}&mtime=111&gen=${cur}`,
    );
    ...
    const stale = await get2(
      `path=${encodeURIComponent(path2)}&mtime=111&gen=${cur - 1}`,
    );
  ...
  it("gives a generation-less read a validator, and answers 304 to it", async () => {
    ...
    const revalidated = await get2(
      `path=${encodeURIComponent(path2)}&mtime=111`,
      {
        ...LOOPBACK,
        "if-none-match": `"${cur}"`,
      },
  ...
  describe("the image route", () => {
    ...
    it("applies the lookup’s tiers: immutable at the current generation, no-cache with current bytes at a superseded one", async () => {
      ...
      expect(
        Buffer.from(await superseded.arrayBuffer()).equals(
          Buffer.from(png2, "base64"),
        ),
      ).toBe(true);
    });
    ...
    it("gives a generation-less read the validator tier, and 304s a matching one", async () => {
      ...
      expect(first.headers.get("cache-control")).toBe("no-cache");
      expect(first.headers.get("etag")).toBe(`"${cur}"`);
      const revalidated = await img2(key2, {
        ...LOOPBACK,
        "if-none-match": `"${cur}"`,
      });
      expect(revalidated.status).toBe(304);
    ...
    it("answers not-found, uncacheable, for a miss and for a stale render alike", async () => {
      ...
      const staleJson = (await (
        await get2(`path=${encodeURIComponent(path2)}&mtime=999`)
      ).json()) as ThumbGetResponse;
    ...
    it("bumps the render’s LRU clock exactly as the lookup hit does (D7)", async () => {
      ...
      const res = await img2(key2);
      expect(res.status).toBe(200);
      expect(statSync(pngPath).mtimeMs).toBeGreaterThan(
        before + 60 * 60 * 1000 - 5000,
      );
    });
    ...
    it("is confined as the lookup is: a path the library refuses gets no pixels even when the cache holds them", async () => {
      ...
      try {
        await cache2.put("/escape.stl", {
          mtime: 5,
          png: Buffer.from("escaped"),
        });
        const json = await get2(
          `path=${encodeURIComponent("/escape.stl")}&mtime=5`,
        );
        const image = await img2(
          `path=${encodeURIComponent("/escape.stl")}&mtime=5`,
        );
  ...
  it("moves the generation on every write, whatever the write carried", async () => {
    ...
    const b = (await (
      await put2({ path: path2, mtime: 111, camera: cam2 })
    ).json()) as { gen: number };
    expect(b.gen).toBeGreaterThan(a.gen);
    // An axis.
    const c2 = (await (
      await put2({ path: path2, mtime: 111, axis: "-z" })
    ).json()) as { gen: number };
    ...
    const d = (await (
      await put2({ path: path2, mtime: 111, camera: null })
    ).json()) as { gen: number };
    expect(d.gen).toBeGreaterThan(c2.gen);
    const e = (await (
      await put2({ path: path2, mtime: 111, axis: null })
    ).json()) as { gen: number };
  ...
  it("refuses to cache anything that is not a hit, even for an entry that exists", async () => {
    ...
    const named = await get2(
      `path=${encodeURIComponent(path2)}&mtime=999&gen=${body.gen}`,
    );
...
describe("PUT /api/thumb deletions and conditional writes", () => {
  ...
  const get3 = (q: string) =>
    app3.request(`/api/thumb?${q}`, { headers: LOOPBACK });
  ...
  const read3 = async (): Promise<ThumbGetResponse> =>
    (await (
      await get3(`path=${encodeURIComponent(path3)}&mtime=111`)
    ).json()) as ThumbGetResponse;
  ...
  it("carries a deletion over the wire — null empties the entry, and is not bad pixels", async () => {
    ...
    expect(
      (await put3({ path: path3, mtime: 111, png: png3, camera: cam3 })).status,
    ).toBe(200);
    expect((await read3()).status).toBe("hit");

    const deleted = await put3({
      path: path3,
      mtime: 111,
      png: null,
      camera: null,
  ...
  it("refuses a write whose entry has moved, distinctly from a malformed one", async () => {
    const written = (await (
      await put3({ path: path3, mtime: 111, png: png3 })
    ).json()) as {
      gen: number;
    };
    ...
    const moved = (await (
      await put3({ path: path3, mtime: 111, png: png3, camera: cam3 })
    ).json()) as {
      gen: number;
    };
    expect(moved.gen).toBeGreaterThan(snapshot);

    const refused = await put3({
      path: path3,
      mtime: 111,
      png: null,
      camera: null,
