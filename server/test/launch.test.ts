...
symlinkSync(
  join(external, "lychee.desktop"),
  join(userApps, "lycheeslicer.desktop"),
);
...
function recorder(
  reply: (file: string, args: string[]) => SpawnResult = () => ({
    code: 0,
    stdout: "",
    stderr: "",
  }),
...
function reportWith(
  config = {},
): ReturnType<ReturnType<typeof createLauncher>["report"]> {
...
describe("desktop-entry reader", () => {
  ...
  it("reads a symlinked file entry and resolves its Name", async () => {
    ...
    // symlink finds it.
    expect(lychee).toEqual({
      id: "lycheeslicer.desktop",
      name: "LycheeSlicer",
    });
  });
  ...
  it("filters NoDisplay entries", async () => {
    ...
    expect(stl?.associated.map((a) => a.id)).not.toContain(
      "hidden-plugin.desktop",
    );
  ...
  it("reads the registry fresh on every report", async () => {
    const launcher = createLauncher({
      env: ENV,
      exec: recorder(xdgMime).exec,
      config: {},
    });
    const before =
      (await launcher.report()).types["model/obj"]?.associated ?? [];
...
describe("templates", () => {
  it("runs the builtin launch when no config is present", async () => {
    ...
    await createLauncher({ env: ENV, exec, config: {} }).launch(
      "lycheeslicer.desktop",
      "/m/a.stl",
    );
  ...
  it("parses an overridden query as id<TAB>name lines", async () => {
    const { exec } = recorder((file) =>
      file === "my-default"
        ...
        : {
            code: 0,
            stdout: "one.desktop\tOne\ntwo.desktop\tTwo\n\n",
            stderr: "",
          },
    );
    const config = {
      default: ["my-default", "{mime}"],
      associations: ["my-assoc", "{mime}"],
    };
  ...
  it("formats a failed command as actor, code and reason", async () => {
    ...
    await expect(
      createLauncher({ env: ENV, exec, config: {} }).launch(
        "x.desktop",
        "/m/a.stl",
      ),
  ...
  it("carries the real command’s reason out of a real launch", async () => {
    ...
    const launcher = createLauncher({
      env: ENV,
      config: {
        launch: ["sh", "-c", 'echo "no such application" >&2; exit 3'],
      },
    });
  ...
  it("reports an unspawnable launch command with its reason", async () => {
    const exec: ExecFn = () =>
      Promise.reject(new Error("spawn gtk-launch ENOENT"));
    await expect(
      createLauncher({ env: ENV, exec, config: {} }).launch(
        "x.desktop",
        "/m/a.stl",
  ...
  it("pipes output for the queries and nothing else, so a launch cannot outlive its request", async () => {
    ...
    for (const config of [
      {},
      { default: ["q-def", "{mime}"], associations: ["q-assoc", "{mime}"] },
    ]) {
  ...
  it("treats a failing configured query as absence rather than a failed report", async () => {
    ...
    const report = await createLauncher({
      ...
      exec,
      config: {
        default: ["bad-default", "{mime}"],
        associations: ["bad-assoc", "{mime}"],
      },
    }).report();
  ...
  it("treats an unspawnable configured query as absence too", async () => {
    const exec: ExecFn = () =>
      Promise.reject(new Error("spawn bad-default ENOENT"));
    const report = await createLauncher({
      env: ENV,
      exec,
      config: {
  ...
  it("treats a missing xdg-mime as no default rather than a failed report", async () => {
    const exec: ExecFn = () =>
      Promise.reject(new Error("spawn xdg-mime ENOENT"));
    const report = await createLauncher({
      env: ENV,
      exec,
      config: {},
...
describe("chooser", () => {
  it("is unavailable with no configuration, and spawns nothing", async () => {
    ...
    await expect(launcher.chooser("/m/a.stl")).rejects.toThrow(
      /no chooser is configured/,
    );
...
describe("config loading", () => {
  ...
  it("falls back to the builtins when the file is absent", () => {
    expect(
      loadLaunchConfig({
        MODEL_BROWSER_LAUNCH_CONFIG: join(cfgDir, "nope.json"),
      }),
    ).toEqual({});
  });
  ...
  it("defaults the path to ~/.config/model-browser/launch.json", () => {
    ...
    try {
      expect(loadLaunchConfig({ HOME: fakeHome })).toEqual({
        chooser: ["from-default-path"],
      });
    } finally {
