...
writeFileSync(
  ...
  zipSync({
    ...
    "inner.zip": [
      new Uint8Array(zipSync({ "deep.stl": new Uint8Array(stlBytes(8)) })),
      { level: 0 },
    ],
...
describe("flat listing of a directory", () => {
  it("lists top-level containers, then all models by relative path in basename order", async () => {
    ...
    expect(body.entries.slice(0, 5).every((e) => e.kind !== "model")).toBe(
      true,
    );
  ...
  it("same-named parts sort together, tie broken by relative path", async () => {
    const body = await flat(root);
    const brackets = body.entries
      .filter((e) => e.name.endsWith("bracket.stl"))
      .map((e) => e.name);
...
describe("bounding", () => {
  ...
  it("charges for every entry examined, not just models and entered dirs", async () => {
    ...
    for (let i = 0; i < 10; i++)
      writeFileSync(join(dir, "sub", `t${i}.txt`), "x");
...
describe("the two reasons a listing is truncated, which the wire does not tell apart", () => {
  ...
  it("the folder cap is capping too — a different bound, the same kind of fact", async () => {
    ...
    const { listing, budgetExhausted, capped } = await walkFlat(
      library,
      root2,
      "set",
    );
...
describe("deep name search (q parameter)", () => {
  ...
  it("a directory reached through a symlink is findable by the name it was reached by", async () => {
    ...
    expect(aliased.entries.map((e) => `${e.kind}:${e.name}`)).toContain(
      "dir:alias",
    );
  ...
  it("queried results order by relative path, keeping each folder contiguous", async () => {
    ...
    expect(
      body.entries.filter((e) => e.kind === "model").map((e) => e.name),
    ).toEqual([
  ...
  it("folders=false narrows the model predicate to the file name", async () => {
    ...
    expect(off.entries.map((e) => `${e.kind}:${e.name}`)).toEqual([
      "dir:SetDunes/spares",
    ]);
  });
  ...
  it("folders=false leaves a file-name match alone", async () => {
    ...
    expect(off.entries.map((e) => e.name)).toEqual([
      "SetDunes/spares/clip.stl",
    ]);
  });
  ...
  it("the option filters the gathered tree; it does not reach the walk", async () => {
    ...
    const containers = (l: DirListing) =>
      l.entries.filter((e) => e.kind !== "model").map((e) => e.name);
  ...
  it("a blank or whitespace-only query is a plain flat listing", async () => {
    ...
    expect(blank.entries.map((e) => e.name)).toEqual([
      ...CONTAINERS,
      ...MODELS,
    ]);
    const whitespace = await flat(root, "true", "   ");
    expect(whitespace.entries.map((e) => e.name)).toEqual([
      ...CONTAINERS,
      ...MODELS,
    ]);
  });
