...
for (let i = 1; i <= 5; i++)
  writeFileSync(join(libTop, "folders", "aa", `${i}.stl`), stlBytes(i));
...
for (let i = 1; i <= 80; i++) {
  writeFileSync(
    join(libTop, "budget", `${String(i).padStart(2, "0")}.txt`),
    "x",
  );
}
...
for (let i = 0; i < 100; i++) {
  wideSub[`bud/sub/${String(i).padStart(3, "0")}.stl`] = new Uint8Array(
    stlBytes(40 + i),
  );
}
writeFileSync(
  ...
  zipSync({
    ...
    "nest/inner.zip": [
      new Uint8Array(zipSync({ "deep.stl": new Uint8Array(stlBytes(33)) })),
      { level: 0 },
    ],
...
for (let i = 99; i >= 0; i--) {
  revEntries[`wide/${String(i).padStart(3, "0")}.stl`] = new Uint8Array(
    stlBytes(140 + i),
  );
}
...
beforeAll(async () => {
  ...
  expect(
    (await app.request("/api/library", { headers: LOOPBACK })).status,
  ).toBe(200);
});
...
async function ask(path: string, n?: number): Promise<Response> {
  ...
  return app.request(`/api/peek?path=${encodeURIComponent(path)}${q}`, {
    headers: LOOPBACK,
  });
}
...
describe("the order a peek walks in", () => {
  it("takes a level's own models before descending, both in sorted order", async () => {
    ...
    expect(names(await peekOf("/kit", 4))).toEqual([
      "a.stl",
      "b.stl",
      "q.stl",
      "y.stl",
  ...
  it("previews the first subfolder in order when the level holds only folders", async () => {
    expect(names(await peekOf("/folders", 4))).toEqual([
      "1.stl",
      "2.stl",
      "3.stl",
      "4.stl",
...
describe("the bound", () => {
  it("cuts a wide level by code-point order, whatever order readdir gave", async () => {
    ...
    rd.reverse = true;
    try {
      expect(names(await peekOf("/wide", 4))).toEqual([
        "m00.stl",
        "m01.stl",
        "m02.stl",
        "m03.stl",
...
describe("what a peek does not enter", () => {
  ...
  it("carries library-root's confinement in its own recursion, not only in listFsDir", async () => {
    ...
    const src = await readFile(
      new URL("../src/listing.ts", import.meta.url),
      "utf8",
    );
    const peekSrc = src.slice(
      src.indexOf("async function peekLevel"),
      src.indexOf("export async function peek"),
    );
  ...
  it("skips hidden subdirectories", async () => {
    ...
    expect(names(await peekOf("/kit", 8))).toEqual([
      "a.stl",
      "b.stl",
      "q.stl",
      "y.stl",
  ...
  it("descends an interior level that holds only subdirectories", async () => {
    expect(names(await peekOf(`${deepZipLibPath}!/onlydirs`, 4))).toEqual([
      "1.stl",
      "2.stl",
    ]);
  });
  ...
  it("skips a nested archive inside an interior rather than descending it", async () => {
    ...
    expect(names(await peekOf(`${deepZipLibPath}!/nest`, 4))).toEqual([
      "ok.stl",
    ]);
  });
  ...
  it("orders an interior level by code point, not by collation", async () => {
    ...
    expect(names(await peekOf(`${deepZipLibPath}!/ord`, 4))).toEqual([
      "B.stl",
      "a.stl",
    ]);
  });
  ...
  it("skips dot-names inside an archive, as a filesystem level does", async () => {
    ...
    expect(names(await peekOf(`${macZipLibPath}!/kit`, 4))).toEqual([
      "part.stl",
    ]);
  ...
  it("carries the archive's mtime on interior finds, so a cell shares the model's cache entry", async () => {
    ...
    const listed = (await (
      await app.request(
        `/api/dir?path=${encodeURIComponent(`${zipLibPath}!/parts`)}`,
        {
          headers: LOOPBACK,
        },
      )
...
describe("the route", () => {
  ...
  it("refuses an n that is not a positive integer", async () => {
    for (const n of ["abc", "0", "-1", "1.5", "", "Infinity"]) {
      const res = await app.request(
        `/api/peek?path=/kit&n=${encodeURIComponent(n)}`,
        {
          headers: LOOPBACK,
        },
      );
  ...
  it("is a path route: the not-ready state envelope, like every other", async () => {
    ...
    const res = await bare.request("/api/peek?path=/kit", {
      headers: LOOPBACK,
    });
