...
describe("semantic index availability", () => {
  ...
  it("a library whose drive is unplugged is volume-gone, not warming", async () => {
    ...
    stubIndex({
      ...
      volume: {
        present: false,
        root: "/run/media/masa/NOPE",
        missing: "/run/media/masa/NOPE",
      },
      failure: {
  ...
  it("a load error while warming is wedged, and its reason and hint are carried", async () => {
    stubIndex({
      ...
      volume: { present: true, root, missing: null },
      failure: {
        reason: "cache built with different settings",
        hint: "rerun classify_stls.py",
        kind: "CacheMismatch",
      },
    });
    const body = (await (
    ...
    expect(body.detail).toBe(
      "cache built with different settings — rerun classify_stls.py",
    );
  });
  ...
  it("availability is cached, not probed per query", async () => {
    ...
    const calls = () =>
      ...
      ).mock.calls.filter((c) => String(c[0]).endsWith("/status")).length;
    const first = calls();
...
describe("semantic query", () => {
  ...
  it("returns tiles built from this server’s own view of the tree", async () => {
    ...
    stubIndex(READY, {
      ...result,
      scope: { ...result.scope, path: join(root, "kits") },
    });
  ...
  it("a hit’s tile carries what the thumbnail cache knows, as a listing’s does", async () => {
    ...
    const first = (await (await post({ text: "dragon" })).json()) as {
      entries: { mtime: number }[];
    };
    const mtime = first.entries[0]!.mtime;
    const gen = await cache.put("/dragon.stl", {
      mtime,
      png: Buffer.from("px"),
      lighting: "camera",
      rig: 1,
    ...
    const body = (await (await post({ text: "dragon" })).json()) as {
      entries: {
        path: string;
        thumb?: { gen: number; ao?: { state: string; rig?: number } };
      }[];
    };
  ...
  it("a hit whose pose is not a pose still becomes a tile, with no pose", async () => {
    ...
    for (const pose of [
      "face-up",
      { ...hit("x.stl").pose, up: [0, 1] },
      0,
      [],
    ]) {
      stubIndex(READY, {
        ...result,
        results: [{ ...hit("dragon.stl"), pose }],
      });
      const body = (await (await post({ text: "dragon" })).json()) as {
        entries: { path: string }[];
  ...
  it("drops a hit that no longer resolves without failing the search", async () => {
    stubIndex(READY, {
      ...result,
      results: [hit("dragon.stl"), hit("moved-away.stl")],
    });
    const body = (await (await post({ text: "dragon" })).json()) as {
      entries: unknown[];
      scores: Record<string, unknown>;
  ...
  it("stats once per returned hit — the bound that lets the two caches disagree", async () => {
    const cost = async (hits: number): Promise<number> => {
      stubIndex(READY, {
        ...result,
        results: Array.from({ length: hits }, () => hit("dragon.stl")),
      });
      stats.n = 0;
      await post({ text: "dragon" });
      return stats.n;
  ...
  it("drops a hit that escapes the library, and keeps one that only aliases inside it", async () => {
    ...
    // no number anywhere.
    expect(Object.keys(body.poses).sort()).toEqual([
      "/alias.stl",
      "/dragon.stl",
    ]);
    ...
    const file = await app.request(
      `/api/file?path=${encodeURIComponent("/escape.stl")}`,
      {
        headers: LOOPBACK,
      },
    );
  ...
  it("reports the index’s own ceiling, distinct from a ranking having more", async () => {
    ...
    const body = (await (await post({ text: "dragon", top: 900 })).json()) as {
      capped: boolean;
    };
  ...
  it("forwards what the count cut from, so a capped view can say what it sampled", async () => {
    ...
    const body = (await (
      await post({ text: "dragon", minScore: 0.1, top: 60 })
    ).json()) as {
      matched?: number;
    };
  ...
  it("a 200 missing the answer’s own fields is an index that is not answering", async () => {
    ...
    for (const body of [
      {},
      { scope: result.scope, weak: false, results: 5 },
      { results: [] },
    ]) {
      resetIndexStatus();
  ...
  it("a hit naming a file that is not a model is dropped, never a model entry without a format", async () => {
    ...
    stubIndex(READY, {
      ...result,
      results: [hit("notes.txt"), hit("kits/a.stl")],
    });
    const res = await post({ text: "notes" });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
      entries: { path: string; kind: string; format?: string }[];
    };
  ...
  it("an index that refuses the request answers as a refusal, not as unavailability", async () => {
    ...
    stubIndex(
      READY,
      { detail: "pool must be one of mean, max, softmax" },
      { queryStatus: 400 },
    );
...
describe("an index answering null roots", () => {
  ...
  it("leaves the peek the plain walk it always was", async () => {
    ...
    const res = await app.request(
      `/api/peek?path=${encodeURIComponent("/kits")}`,
      {
        headers: LOOPBACK,
      },
    );
