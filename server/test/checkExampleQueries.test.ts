 ...
 */
function fakeIndex(opts: { empty?: string[]; status?: Record<string, number> } = {}) {
  const sent: { url: string; body: { text?: string; top?: number; minScore?: number; path?: string } }[] = []
  const fetchFn: FetchLike = async (input, init) => {
    ...
    if (status !== undefined) return new Response('the index fell over', { status })
    const entries = opts.empty?.includes(text) === true ? [] : [{ path: `/kit/${text}.stl`, name: 'a model' }]
...
describe('checkExampleQueries', () => {
  ...
  it('counts a thrown request as failed too, and asks the rest anyway', async () => {
    ...
    expect(sent.map((s) => s.body.text)).toEqual(['a treasure chest', 'a stone golem'])
  })
  ...
  it('sends each query to /api/semantic under the visitor’s own options, and no path', async () => {
    ...
    expect(sent.map((s) => s.url)).toEqual(Array(3).fill(`${ORIGIN}/api/semantic`))
  ...
  it('answers both lists empty when every query is alive', async () => {
    ...
    await expect(checkExampleQueries(ORIGIN, THREE, fetchFn)).resolves.toEqual({ dead: [], failed: [] })
  })
})
