...
writeFileSync(
  join(dir, "a.zip"),
  zipSync({ "part.stl": new Uint8Array(aPart) }),
);
writeFileSync(
  join(dir, "b.zip"),
  zipSync({ "part.stl": new Uint8Array(bPart) }),
);
writeFileSync(
  join(dir, "nest.zip"),
  zipSync({
    "inner.zip": [
      zipSync({ "deep.stl": new Uint8Array(stlBytes(7)) }),
      { level: 0 },
    ],
  }),
);
...
function harness(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    XDG_DATA_DIRS: join(dir, "no-such-share"),
  };
  const app = createApp(
...
describe("a deployment that withholds the launcher", () => {
  ...
  it("is the field talking, not the routes: with it on, the same launcher is reached", async () => {
    ...
    expect((await app.request("/api/apps", { headers: LOOPBACK })).status).toBe(
      200,
    );
    expect(
    ...
    expect(
      (await post(app, "/api/open-with", { path: "/loose.stl" })).status,
    ).toBe(200);
    expect(calls).toEqual([
      "report",
      `launch x.desktop ${join(dir, "loose.stl")}`,
      `chooser ${join(dir, "loose.stl")}`,
    ]);
  });
});
...
describe("GET /api/apps", () => {
  it("reports every handled type, and whether a chooser is configured", async () => {
    ...
    expect(Object.keys(body.types).sort()).toEqual([
      "model/3mf",
      "model/obj",
      "model/stl",
    ]);
...
describe("POST /api/open", () => {
  ...
  it("requires a path and an appId", async () => {
    const { calls, app } = harness();
    expect((await post(app, "/api/open", { appId: "x.desktop" })).status).toBe(
      400,
    );
    expect((await post(app, "/api/open", { path: "/loose.stl" })).status).toBe(
      400,
    );
  ...
  it("runs the configured argv in place of the builtin", async () => {
    const { calls, app } = harness({
      launch: ["my-opener", "--app", "{appId}", "--file", "{file}"],
    });
    await post(app, "/api/open", { path: "/loose.stl", appId: "x.desktop" });
  ...
  it("surfaces a failing launch command with its reason", async () => {
    ...
    expect(((await res.json()) as { error: string }).error).toMatch(
      /exited 4: no such application/,
    );
  });

  it.skipIf(asRoot)(
    "names an unreadable archive by its library path, and spawns nothing",
    async () => {
      ...
      expect(await res.json()).toEqual({
        error: "cannot read zip: /locked.zip",
      });
      expect(calls).toHaveLength(0);
    },
  );
  ...
  it("extracts a zip-entry launch into the injected root, never the bare system tmpdir (4.5)", async () => {
    ...
    const before = new Set(
      readdirSync(tmpdir()).filter((n) => n.startsWith("model-browser-open-")),
    );
    ...
    const after = readdirSync(tmpdir()).filter((n) =>
      n.startsWith("model-browser-open-"),
    );
...
describe("POST /api/open-with", () => {
  it("reports unavailable, spawning nothing, when no chooser is configured", async () => {
    ...
    expect(await res.json()).toEqual({
      error: "no chooser is configured",
      unavailable: true,
    });
    expect(calls).toHaveLength(0);
  });
  ...
  it("rejects a nested zip entry and a relative path without spawning", async () => {
    const { calls, app } = harness({ chooser: ["open-with", "{file}"] });
    expect(
      (await post(app, "/api/open-with", { path: "/nest.zip!/inner.zip" }))
        .status,
    ).toBe(400);
    expect(
      (await post(app, "/api/open-with", { path: "loose.stl" })).status,
