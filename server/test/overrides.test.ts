...
function v1(entries: Record<string, unknown>): {
  version: number;
...
describe("loading the store", () => {
  ...
  it("drops a wrong-typed modified, reports it, and keeps the rest", async () => {
    ...
    const { store, problems } = await loadWith(
      v1({
        "/kit": {
          credits: { ...CREDITS, licenseUrl: LICENSE_URL, modified: true },
        },
      }),
    );
    expect(resolveOverrides(store, "/kit")).toEqual({
  ...
  it("strips a trailing slash from a key’s archive-entry half, and lookups match it", async () => {
    ...
    const { store } = await loadWith(
      v1({ "/kit/a.zip!/parts/": { credits: CREDITS } }),
    );
    expect([...store.keys()]).toEqual(["/kit/a.zip!/parts"]);
    expect(resolveOverrides(store, "/kit/a.zip!/parts/x.stl")).toEqual({
      credits: CREDITS,
    });
  });
...
describe("field-wise longest-prefix resolution", () => {
  it("carries a kit’s credits down to a file beneath it", async () => {
    const { store } = await loadWith(v1({ "/kit": { credits: CREDITS } }));
    expect(resolveOverrides(store, "/kit/sub/x.stl")).toEqual({
      credits: CREDITS,
    });
  });
  ...
  it("merges per field: a file key holding only a pose keeps the kit’s credits", async () => {
    ...
    expect(resolveOverrides(store, "/kit/x.stl")).toEqual({
      credits: CREDITS,
      pose,
    });
  });
  ...
  it("does not inherit a name — it labels the kit alone", async () => {
    const { store } = await loadWith(
      v1({ "/kit": { name: "Player Character Pack 03", credits: CREDITS } }),
    );
    expect(resolveOverrides(store, "/kit")).toEqual({
  ...
  it("resolves a name from the entry’s own key", async () => {
    const { store } = await loadWith(
      v1({ "/kit": { name: "Kit" }, "/kit/x.stl": { name: "Hero" } }),
    );
  ...
  it("replaces credits as a block: a partial block beneath a kit key drops the rest, it does not merge", async () => {
    ...
    expect(resolveOverrides(store, "/kit/hero.stl")).toEqual({
      credits: partial,
    });
    expect(resolveOverrides(store, "/kit/other.stl")).toEqual({
      credits: CREDITS,
    });
  });
  ...
  it("resolves a zip-root lookup exactly as the archive file’s own path does", async () => {
    const { store } = await loadWith(
      v1({ "/kit/a.zip": { name: "The Archive", credits: CREDITS } }),
    );
  ...
  it("lets the root key cover everything, name excepted", async () => {
    const { store } = await loadWith(
      v1({ "/": { name: "Library", credits: CREDITS } }),
    );
    expect(resolveOverrides(store, "/")).toEqual({
      name: "Library",
      credits: CREDITS,
    });
    expect(resolveOverrides(store, "/kit/x.stl")).toEqual({ credits: CREDITS });
...
describe("the store’s lifetime", () => {
  ...
  function markerAt(dir: string, id: string): void {
    ...
    writeFileSync(
      join(dir, MARKER_DIR, "library.json"),
  ...
  it("reads the file once while the library stays resolved", async () => {
    ...
    const holder = createOverrideHolder(
      libraryOn({
        MODEL_BROWSER_ROOT: top,
        HOME: top,
        XDG_CONFIG_HOME: join(top, "cfg"),
      }),
    );
  ...
  it("resolves nothing while the library is not ready, and loads once it is", async () => {
    ...
    storeAt(top, v1({ "/kit": { credits: CREDITS } }));
    const env: NodeJS.ProcessEnv = {
      MODEL_BROWSER_CONFIG: join(top, "absent.json"),
    };
    const library = libraryOn(env);
    const holder = createOverrideHolder(library);
...
describe("writing the store", () => {
  it("round-trips through the loader", async () => {
    ...
    await writeOverrides(top, {
      version: 1,
      entries: { "/kit": { credits: CREDITS } },
    });
...
describe("GET /api/overrides", () => {
  ...
  it("answers the resolved credits for an entry beneath a kit key", async () => {
    ...
    const res = await appOn(libTop).request(
      "/api/overrides?path=/kit/sub/y.stl",
      {
        headers: LOOPBACK,
      },
    );
  ...
  it("answers an empty object where nothing resolves", async () => {
    ...
    const res = await appOn(libTop).request("/api/overrides?path=/kit2/y.stl", {
      headers: LOOPBACK,
    });
    expect(res.status).toBe(200);
  ...
  it("answers a marker path exactly as the library does, as one that is not there", async () => {
    ...
    expect(await res.json()).toEqual({
      error: `no such path: /${MARKER_DIR}/overrides.json`,
    });
  });
  ...
  it("answers the not-ready envelope while the library is unconfigured", async () => {
    ...
    const res = await app.request("/api/overrides?path=/kit", {
      headers: LOOPBACK,
    });
...
describe("GET /api/credits", () => {
  it("answers the store’s credited kits, carrying library paths and no location on the host", async () => {
    ...
    const res = await appOn(libTop).request("/api/credits", {
      headers: LOOPBACK,
    });
    expect(res.status).toBe(200);
  ...
  it("answers an empty list for a library with no store", async () => {
    const res = await appOn(fixtureLibrary()).request("/api/credits", {
      headers: LOOPBACK,
    });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual([]);
...
describe("display names ride the listing", () => {
  ...
  it("labels a named directory in a browse, and nothing beneath it", async () => {
    ...
    const root = (await (
      await app.request("/api/dir?path=/", { headers: LOOPBACK })
    ).json()) as DirListing;
    expect(named(root.entries, "/kit").displayName).toBe(
    ...
    const sub = (await (
      await app.request("/api/dir?path=/kit/sub", { headers: LOOPBACK })
    ).json()) as DirListing;
  ...
  it("labels a named entry in a flat listing", async () => {
    const libTop = fixtureLibrary();
    storeAt(libTop, NAMES);
    const res = await appOn(libTop).request("/api/dir?path=/&flat=true", {
      headers: LOOPBACK,
    });
    const listing = (await res.json()) as DirListing;
    expect(named(listing.entries, "/kit").displayName).toBe(
      "Player Character Pack 03",
    );
    expect(named(listing.entries, "/kit/x.stl").displayName).toBe(
      "Kindle Cleric",
    );
    ...
    expect(
      named(listing.entries, "/kit/sub/y.stl").displayName,
    ).toBeUndefined();
  });
  ...
  it("labels a named entry in a deep-search listing", async () => {
    ...
    const res = await appOn(libTop).request("/api/dir?path=/&flat=true&q=sub", {
      headers: LOOPBACK,
    });
    const listing = (await res.json()) as DirListing;
  ...
  it("labels a named entry in a peek answer", async () => {
    ...
    const libTop = fixtureLibrary();
    storeAt(libTop, NAMES);
    const res = await appOn(libTop).request("/api/peek?path=/kit", {
      headers: LOOPBACK,
    });
  ...
  it("labels an entry inside an archive", async () => {
    const libTop = fixtureLibrary();
    storeAt(libTop, NAMES);
    const res = await appOn(libTop).request("/api/dir?path=/kit/a.zip", {
      headers: LOOPBACK,
    });
    const listing = (await res.json()) as DirListing;
    expect(named(listing.entries, "/kit/a.zip!/parts").displayName).toBe(
      "Loose Parts",
    );
    expect(
      named(listing.entries, "/kit/a.zip!/box.stl").displayName,
    ).toBeUndefined();
  });
  ...
  it("emits identical listings for a library with no store", async () => {
    ...
    for (const path of ["/", "/kit", "/kit/a.zip"]) {
      ...
      const bare = (await (
        await appOn(without).request(url, { headers: LOOPBACK })
      ).json()) as DirListing;
    ...
    const before = (await (
      await appOn(without).request("/api/dir?path=/", { headers: LOOPBACK })
    ).json()) as DirListing;
    storeAt(without, NAMES);
    const after = (await (
      await appOn(without).request("/api/dir?path=/", { headers: LOOPBACK })
    ).json()) as DirListing;
    expect(named(after.entries, "/kit").displayName).toBe(
      "Player Character Pack 03",
    );
