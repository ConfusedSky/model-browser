...
function markerAt(dir: string, id: string): void {
  ...
  writeFileSync(
    join(dir, ".model-browser", "library.json"),
...
function libraryAt(root: string, extra: NodeJS.ProcessEnv = {}) {
  return createLibrary({
    MODEL_BROWSER_ROOT: root,
    MODEL_BROWSER_CONFIG: join(tempTree(), "absent.json"),
    ...extra,
  });
}
...
describe("library discovery", () => {
  ...
  it("writes a marker at a root with none above it, and reads it back next time", async () => {
    ...
    expect(
      JSON.parse(
        readFileSync(join(root, ".model-browser", "library.json"), "utf8"),
      ),
    ).toEqual({
      id: first.id,
      version: 1,
  ...
  it("keeps walking past a malformed marker and past one whose id is not a string", async () => {
    ...
    writeFileSync(
      join(idless, ".model-browser", "library.json"),
      "{ not json at all",
    );
    mkdirSync(join(broken, ".model-browser"));
    writeFileSync(
      join(broken, ".model-browser", "library.json"),
      JSON.stringify({ id: 7, version: 1 }),
    );
  ...
  it("serves an unwritable top under a hashed id and says it is unmarked", async () => {
    ...
    if (process.getuid?.() === 0) {
      ...
      console.warn(
        "skipped: running as root, which may write into a 0555 directory",
      );
      return;
    }
...
describe("library configuration", () => {
  it("takes the root from the environment over the configuration", async () => {
    ...
    const state = await createLibrary(
      { MODEL_BROWSER_ROOT: chosen },
      { root: ignored },
    ).state();
  ...
  it("is unconfigured with neither", async () => {
    expect(await createLibrary({}, {}).state()).toEqual({
      state: "unconfigured",
    });
  ...
  it("refresh re-evaluates a ready library", async () => {
    ...
    expect((await library.state()).state === "ready" && library.id()).toBe(
      "first-id",
    );
  ...
  it("takes a bare directory at the mount point as a new library rather than the one that left", async () => {
    ...
    expect(
      JSON.parse(
        readFileSync(join(top, ".model-browser", "library.json"), "utf8"),
      ).id,
    ).toBe(state.state === "ready" ? state.id : undefined);
  });
  ...
  it("answers a burst of first calls with one evaluation, and writes one marker", async () => {
    ...
    const ids = [
      ...new Set(states.map((s) => (s.state === "ready" ? s.id : s.state))),
    ];
  ...
  it("answers a burst across a return transition with the tree that arrived", async () => {
    ...
    expect(states).toEqual(
      Array(4).fill({ state: "ready", id: "second-drive", top, root: "/" }),
    );
  });
...
describe("the marker walk stops at a mount boundary", () => {
  it("does not adopt a marker across a device change, and still takes one below it", async () => {
    ...
    expect(await findMarker(start, devOf)).toEqual({
      top: boundary,
      id: "this-side",
    });
  });
...
describe("a root above an existing library", () => {
  ...
  it("names the shallowest library below the root", async () => {
    ...
    expect(await libraryAt(root).state()).toEqual({
      state: "nested",
      root,
      library: shallow,
    });
  ...
  it("reaches four levels down but not five", async () => {
    ...
    expect(await libraryAt(four).state()).toEqual({
      state: "nested",
      root: four,
      library: atFour,
    });
  ...
  it("gives up on a tree too wide to reach the depth the library sits at", async () => {
    ...
    for (let i = 0; i < 2100; i++)
      markerAt(join(wide, `kit-${i}`, "inner"), `past-the-cap-${i}`);
    ...
    for (let i = 0; i < 3; i++)
      markerAt(join(narrow, `kit-${i}`, "inner"), `within-the-cap-${i}`);
  ...
  it("checks every direct child of the root, wherever it was listed", async () => {
    ...
    for (const marked of ["kit-599", "kit-0"]) {
      const wide = join(tempTree(), "drive");
      for (let i = 0; i < 600; i++)
        mkdirSync(join(wide, `kit-${i}`), { recursive: true });
...
async function readyLibrary(): Promise<{
  tmp: string;
  top: string;
  library: ReturnType<typeof libraryAt>;
}> {
  const tmp = tempTree();
  const top = join(tmp, "lib");
...
describe("library resolve", () => {
  it("folds `..` against the top instead of escaping through it", async () => {
    ...
    expect(await library.resolve("/a/../../etc")).toEqual({
      fsPath: join(top, "etc"),
      entry: undefined,
    });
    expect(await library.resolve("/kits/../kits/a")).toEqual({
      fsPath: join(top, "kits", "a"),
      entry: undefined,
    });
  });
  ...
  it("reads a filesystem-looking path as the library path it is", async () => {
    const { top, library } = await readyLibrary();
    expect(await library.resolve("/etc/passwd")).toEqual({
      fsPath: join(top, "etc", "passwd"),
      entry: undefined,
    });
  });
  ...
  it("resolves a path that does not exist without stat-ing or throwing", async () => {
    ...
    expect(await library.resolve("/")).toEqual({
      fsPath: top,
      entry: undefined,
    });
  });
  ...
  it("refuses a symlink whose target is outside and follows one whose target is inside", async () => {
    ...
    await expect(library.resolve("/escape")).rejects.toMatchObject({
      status: 400,
    });
    await expect(library.resolve("/escape/secret.stl")).rejects.toMatchObject({
      status: 400,
    });
    // Inside: allowed, and what comes back is the target's real path.
    expect(await library.resolve("/alias/a")).toEqual({
      fsPath: join(top, "kits", "a"),
      entry: undefined,
    });
  });
  ...
  it("decides a nonexistent path on its nearest existing ancestor", async () => {
    ...
    await expect(library.resolve("/escape/gone.stl")).rejects.toMatchObject({
      status: 400,
    });
  });
  ...
  it("confines a virtual path by its archive half and leaves the entry half untouched", async () => {
    ...
    expect(await library.resolve("/kits/a/parts.zip!/x//y.stl")).toEqual({
      fsPath: zip,
      entry: "x//y.stl",
    });
    ...
    expect((await library.resolve("/kits/a/parts.zip!/../x.stl")).entry).toBe(
      "../x.stl",
    );
  });
  ...
  it("refuses a virtual path whose archive is outside the library", async () => {
    ...
    await expect(library.resolve("/parts.zip!/x.stl")).rejects.toMatchObject({
      status: 400,
    });
  });
  ...
  it("names no filesystem detail in any refusal", async () => {
    ...
    for (const path of ["a/b", "/escape", "/escape/gone.stl"]) {
      messages.push(
        await library.resolve(path).then(
          () => "",
          (e: Error) => e.message,
        ),
      );
...
describe("library libPathOf", () => {
  ...
  it("refuses a real path outside the top, including a sibling with the same prefix", async () => {
    const { tmp, top, library } = await readyLibrary();
    expect(() => library.libPathOf(join(tmp, "elsewhere"))).toThrow(
      LibraryError,
    );
    try {
      library.libPathOf(`${top}-next-door`);
      expect.unreachable(
        "a sibling directory sharing the prefix is not inside the library",
      );
    } catch (err) {
