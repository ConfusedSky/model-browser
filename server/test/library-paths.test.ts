...
describe("nothing outside the library is listed", () => {
  ...
  it("omits them from a flat walk, and enumerates no name from the escaping archive", async () => {
    ...
    expect(
      names.filter((n) => n.includes("secret") || n.includes("hidden")),
    ).toEqual([]);
  ...
  it("still lists an alias that stays inside, under its own route", async () => {
    ...
    expect(inside.entries.find((e) => e.name === "alias")?.path).toBe(
      "/in/alias",
    );
    ...
    expect(found.entries.map((e) => `${e.kind}:${e.name}`)).toContain(
      "dir:in/alias",
    );
  });
  ...
  it("refuses an escaping thumbnail read and write alike", async () => {
    const read = await get(
      `/api/thumb?path=${encodeURIComponent("/in/escape.stl")}&mtime=1`,
    );
...
describe("a hidden entry is unreachable, not merely unlisted", () => {
  ...
  it("answers a hidden component as a path that does not exist, whatever is really there", async () => {
    ...
    for (const [path, canonical] of paths) {
      ...
      for (const url of cases) {
        ...
        expect([url, await res.json()]).toEqual([
          url,
  ...
  it("answers it indistinguishably from a path that is simply not there", async () => {
    ...
    for (const route of ["/api/dir", "/api/peek"]) {
      const hidden = await get(
        `${route}?path=${encodeURIComponent("/.trash/x.stl")}`,
      );
      const absent = await get(
        `${route}?path=${encodeURIComponent("/nope/x.stl")}`,
      );
      expect([route, hidden.status]).toEqual([route, absent.status]);
      const shape = (body: { error: string }) =>
        body.error.replace(/\/[^/]+\/x\.stl$/, "/…");
      expect([
        route,
        shape((await hidden.json()) as { error: string }),
      ]).toEqual([route, shape((await absent.json()) as { error: string })]);
    }
    ...
    const file = await get(
      `/api/file?path=${encodeURIComponent("/nope/x.stl")}`,
    );
    expect([file.status, await file.json()]).toEqual([
      404,
      { error: "no such file: /nope/x.stl" },
    ]);
  });
  ...
  it("answers a hidden component nested under a directory that is browsable", async () => {
    ...
    const res = await get(
      `/api/file?path=${encodeURIComponent("/kit/.private/secret.stl")}`,
    );
    expect(res.status).toBe(404);
    expect(await res.json()).toEqual({
      error: "no such path: /kit/.private/secret.stl",
    });
    // The control, one component apart: the model beside it is served.
    expect(
      (await get(`/api/file?path=${encodeURIComponent("/kit/part.stl")}`))
        .status,
    ).toBe(200);
  });
});
...
describe("a path built to cost rather than to name a file", () => {
  ...
  it("walks a path at the component bound and refuses one past it", async () => {
    expect(await status("/a".repeat(256))).toEqual([
      404,
      "no such path: " + "/a".repeat(256),
    ]);
  ...
  it("walks a path at the byte bound and refuses one past it", async () => {
    ...
    expect(await status(`/${"a".repeat(4096)}`)).toEqual([
      400,
      "path too long",
    ]);
  });
