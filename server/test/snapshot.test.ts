...
function treeFileOf(base: string, id: string, root: string): string {
  return join(
    ...
    id,
    SNAPSHOT_DIR,
    `tree-${createHash("sha256").update(root).digest("hex")}.json`,
  );
}
...
describe("the snapshot store", () => {
  it("round-trips a snapshot for a root", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-round")),
    );
  ...
  it("answers null for a root it has never walked", async () => {
    const store = new SnapshotStore(
      cacheRoot(),
      undefined,
      libraryFor(makeLibraryTree("lib-miss")),
    );
  ...
  it("keeps each walked root in its own file", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-roots")),
    );
    await store.save(snapshotOf("/kits"));
    ...
    expect(readdirSync(dir).filter((f) => f.startsWith("tree-")).length).toBe(
      2,
    );
  });
  ...
  it("drops the snapshot a revalidation contradicted, and leaves its siblings", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-inval")),
    );
    await store.save(snapshotOf("/kits"));
    await store.save(snapshotOf("/other", []));
...
describe("the on-disk format version", () => {
  it("invalidates a snapshot written under another version rather than mis-parsing it", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-version")),
    );
    await store.save(snapshotOf("/kits"));
    ...
    writeFileSync(
      file,
      JSON.stringify({ ...onDisk, version: SNAPSHOT_VERSION + 1 }),
    );
  ...
  it("leaves an unusable file for the sweep rather than deleting it on read", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-pureread")),
    );
    await store.save(snapshotOf("/kits"));
  ...
  it("invalidates an unparseable snapshot", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-garbage")),
    );
    await store.save(snapshotOf("/kits"));
    writeFileSync(
      treeFileOf(base, "lib-garbage", "/kits"),
      "{ this is not json",
    );
    expect(await store.load("/kits")).toBeNull();
  });
...
describe("keying by library identity", () => {
  it("is a hit when the same library is reached at another mount point", async () => {
    ...
    await new SnapshotStore(base, undefined, libraryFor(top)).save(
      snapshotOf("/kits"),
    );
  ...
  it("never lets two libraries with the same layout share a snapshot", async () => {
    ...
    await new SnapshotStore(base, undefined, libraryFor(first)).save(
      snapshotOf("/kits"),
    );

    expect(
      await new SnapshotStore(base, undefined, libraryFor(second)).load(
        "/kits",
      ),
    ).toBeNull();
  });
  ...
  it("refuses a snapshot filed under another library, on its own contents", async () => {
    const base = cacheRoot();
    await new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-owner")),
    ).save(snapshotOf("/kits"));
    ...
    const thief = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-thief")),
    );
...
describe("the archive-directory cache", () => {
  ...
  function libraryWithArchives(
    id: string,
    count: number,
  ): { top: string; zips: string[] } {
    ...
    for (let i = 0; i < count; i++) {
      ...
      writeFileSync(
        path,
        zipSync({ [`part-${i}.stl`]: new Uint8Array(stlBytes(i + 1)) }),
      );
      zips.push(path);
    }
    return { top, zips };
  ...
  it("re-reads and replaces the directory of a rewritten archive", async () => {
    ...
    expect((await listZipEntries(rewritten, cache)).map((e) => e.name)).toEqual(
      ["part-0.stl"],
    );
    ...
    fs.opens.length = 0;
    expect(
      (await listZipEntries(rewritten, cache)).map((e) => e.name).sort(),
    ).toEqual(["extra.stl", "renamed.stl"]);
    // It was re-read, and its neighbour was not disturbed.
    expect(opensOf(rewritten)).toBeGreaterThan(0);
    fs.opens.length = 0;
    expect((await listZipEntries(untouched, cache)).map((e) => e.name)).toEqual(
      ["part-1.stl"],
    );
    ...
    const record = JSON.parse(
      readFileSync(
        join(store.dir, "lib-rewritten", SNAPSHOT_DIR, ARCHIVES_FILE),
        "utf8",
      ),
    );
    expect(
      record.archives["/kit-0.zip"].entries
  ...
  it("keys archives by library path, so a remount hits", async () => {
    ...
    const remounted = new SnapshotStore(base, undefined, libraryFor(elsewhere));
    expect(
      (await listZipEntries(moved, remounted.archiveCache())).map(
        (e) => e.name,
      ),
    ).toEqual(["part-0.stl"]);
...
describe("atomic writes", () => {
  it("leaves the old snapshot whole behind a failed write, never a torn one", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-torn")),
    );
    await store.save(snapshotOf("/kits"));

    const replacement = snapshotOf("/kits", [
      { ...ENTRY, name: "half-written.stl" },
    ]);
    fs.failRename = true;
    await expect(store.save(replacement)).rejects.toThrow(
    ...
    expect(
      readdirSync(join(base, "lib-torn", SNAPSHOT_DIR)).filter((f) =>
        f.endsWith(".tmp"),
      ),
    ).toEqual([]);
  });
  ...
  it("leaves the old archive layer whole behind a failed write", async () => {
    ...
    const onDisk = JSON.parse(
      readFileSync(
        join(base, "lib-torn-zip", SNAPSHOT_DIR, ARCHIVES_FILE),
        "utf8",
      ),
    );
    expect(
      onDisk.archives["/kit.zip"].entries.map((e: { name: string }) => e.name),
    ).toEqual(["a.stl"]);
  });
});
...
describe("the store directory", () => {
  it("is created lazily — a cache from before this change has no snapshots/", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-lazy")),
    );
  ...
  it("tolerates its own absence in every operation", async () => {
    const store = new SnapshotStore(
      cacheRoot(),
      undefined,
      libraryFor(makeLibraryTree("lib-absent")),
    );
...
describe("the size bound", () => {
  ...
  it("leaves a store under its cap entirely alone", async () => {
    ...
    const store = new SnapshotStore(
      base,
      64 * 1024 ** 2,
      libraryFor(makeLibraryTree("lib-under")),
    );
    await store.save(snapshotOf("/kits"));
    await store.maintain();
    expect(await store.load("/kits")).toEqual(snapshotOf("/kits"));
...
describe("the sweep and a write in flight (review finding 8)", () => {
  it("reaps a stale temp file and leaves a live one alone", async () => {
    ...
    const store = new SnapshotStore(
      base,
      undefined,
      libraryFor(makeLibraryTree("lib-tmp")),
    );
    await store.save(snapshotOf("/kits"));
...
describe("the archive layer survives its own flush and the sweep (review finding 9)", () => {
  ...
  it("persists an archive learned while the previous flush was still writing", async () => {
    ...
    const onDisk = JSON.parse(
      readFileSync(
        join(base, "lib-midflush", SNAPSHOT_DIR, ARCHIVES_FILE),
        "utf8",
      ),
    );
    expect(Object.keys(onDisk.archives).sort()).toEqual([
      "/kit-0.zip",
      "/kit-1.zip",
    ]);
  });
  ...
  it("serializes concurrent flushes, so the older write cannot land last", async () => {
    ...
    const onDisk = JSON.parse(
      readFileSync(
        join(base, "lib-flushrace", SNAPSHOT_DIR, ARCHIVES_FILE),
        "utf8",
      ),
    );
    expect(Object.keys(onDisk.archives).sort()).toEqual([
  ...
  it("never evicts the archive layer, whatever the cap says", async () => {
    ...
    expect(
      await new SnapshotStore(base, 0, library).load("/two"),
    ).not.toBeNull();
    ...
    await listZipEntries(
      zips[0]!,
      new SnapshotStore(base, 0, library).archiveCache(),
    );
  ...
  it("drops the layer from memory when the sweep reaps its file", async () => {
    ...
    await store.maintain();
    expect(
      readdirSync(join(base, "lib-reap-layer", SNAPSHOT_DIR)),
    ).not.toContain(ARCHIVES_FILE);
...
describe("living beside the thumbnail cache", () => {
  ...
  it("is invisible to the thumbnail sweep, which still sweeps", async () => {
    ...
    await thumbs.put("/kits/a/deleted.stl", {
      mtime: 1,
      png: Buffer.from("png"),
    });
