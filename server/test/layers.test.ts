...
async function fixture(id: string): Promise<Fixture> {
  ...
  writeFileSync(
    join(top, ".model-browser", "library.json"),
  ...
  return {
    ...
    zip,
    library,
    store: new SnapshotStore(base, undefined, library),
    base,
  };
}
...
function serverFor(
  f: Fixture,
  opts: { store?: SnapshotStore; listings?: ListingCache } = {},
): Server {
...
async function listFlat(s: Server, path = ROOT): Promise<DirListing> {
  const res = await s.app.request(
    `/api/dir?flat=true&path=${encodeURIComponent(path)}`,
    {
      headers: LOOPBACK,
    },
  );
...
async function listDir(s: Server, path: string): Promise<DirListing> {
  const res = await s.app.request(`/api/dir?path=${encodeURIComponent(path)}`, {
    headers: LOOPBACK,
  });
  expect(res.status).toBe(200);
  return (await res.json()) as DirListing;
...
async function askPoses(s: Server, path: string): Promise<Response> {
  return s.app.request(`/api/semantic/poses?path=${encodeURIComponent(path)}`, {
    headers: LOOPBACK,
  });
}
...
describe("the pose layer rides the listing (§6.1, §6.3)", () => {
  ...
  it("drops both layers when the index answers from another collection root (§7.3)", async () => {
    ...
    expect(listing.entries.map((e) => e.name)).toEqual(
      (await listFlat(serverFor(f, { store: undefined }))).entries.map(
        (e) => e.name,
      ),
    );
  });
  ...
  it("a restart drops the layers while the tree keeps serving (the third drop point)", async () => {
    ...
    const restarted = serverFor(f, {
      store: new SnapshotStore(f.base, undefined, f.library),
    });
    ...
    expect(
      await new SnapshotStore(f.base, undefined, f.library).load(ROOT),
    ).not.toBeNull();
  });
});
...
describe("the preview layer and its ancestors (§6.1, §7.3)", () => {
  ...
  it("a sheet carried on the listing has cells annotated as the models beside its folder are", async () => {
    ...
    const cell: DirEntry = {
      name: "bracket.stl",
      path: `${ROOT}/a/bracket.stl`,
      kind: "model",
      size: 1,
      mtime: 2,
    };
    s.listings.layers.recordPreview(f.top, `${ROOT}/a`, 4, [cell]);
    const gen = await s.cache.put(cell.path, {
      mtime: 2,
      png: Buffer.from("px"),
      lighting: "camera",
      rig: 1,
    ...
    expect(
      s.listings.layers.previewFor(`${ROOT}/a`, 4)![0]!.thumb,
    ).toBeUndefined();
  });
  ...
  it("re-derives the changed directory and every ancestor, and no unchanged sibling", async () => {
    ...
    for (const gone of ["/", ROOT, `${ROOT}/a`, `${ROOT}/a/deep`]) {
      expect(
        listings.layers.previewFor(gone, 4),
        `${gone} should be re-derived`,
      ).toBeUndefined();
    }
  ...
  it("a peek records its choice, and a listing then carries it on the folder tile", async () => {
    ...
    const peeked = await s.app.request(
      `/api/peek?path=${encodeURIComponent(`${ROOT}/a`)}&n=4`,
      {
        headers: LOOPBACK,
      },
    );
    ...
    expect(entryFor(listing, "a").preview?.map((e) => e.path)).toEqual(
      sheet.map((e) => e.path),
    );
...
describe("the thumbnail-state index (§6.2)", () => {
  ...
  it("reads staleness against the entry’s own mtime, not against a stored verdict", async () => {
    ...
    expect(entryFor(await listFlat(s), "loose.stl").thumb?.ao?.state).toBe(
      "hit",
    );
  ...
  it("a read teaches it as much as a write does", async () => {
    ...
    // second round trip.
    const got = await s.app.request(
      `/api/thumb?path=${encodeURIComponent("/kit/loose.stl")}&mtime=${mtime}`,
      {
        headers: LOOPBACK,
      },
    );
...
describe("a library with no layer content emits what it always did (§6.3, §7.3)", () => {
  it("is byte-identical to a server that has no layers at all", async () => {
    ...
    const bare = JSON.stringify(
      await listFlat(serverFor(f, { store: undefined })),
    );
    ...
    const bareDir = JSON.stringify(
      await listDir(serverFor(f, { store: undefined }), ROOT),
    );
...
describe("enumerating a scope (§6.7)", () => {
  ...
  function wide(f: Fixture, name: string, n: number): void {
    ...
    for (let i = 0; i < n; i++)
      writeFileSync(join(dir, `m${i}.stl`), stlBytes(i));
  }
  ...
  it("answers a subfolder from an ancestor’s snapshot, named as a walk of it would", async () => {
    ...
    expect(under.entries.map((e) => e.name).sort()).toEqual([
      "bracket.stl",
      "deep/part.stl",
    ]);
  ...
  it("404s a path that only looks like it lies under a cached root (round-2 finding 4a)", async () => {
    ...
    expect(real.entries.map((e) => e.name).sort()).toEqual([
      "bracket.stl",
      "deep/part.stl",
    ]);
  ...
  it("states incompleteness rather than refusing, and caches nothing for a cut traversal", async () => {
    ...
    for (let i = 0; i < 3; i++)
      writeFileSync(join(f.top, "big", "a", `m${i}.stl`), stlBytes(i));
    for (let i = 0; i < 60; i++)
      writeFileSync(join(f.top, "big", "b", `n${i}.stl`), stlBytes(i));
    const s = serverFor(f);
    ...
    expect(models.entries.map((e) => e.name)).toEqual([
      "a/m0.stl",
      "a/m1.stl",
      "a/m2.stl",
    ]);
  ...
  it("is a path route: the not-ready envelope, and the same refusals as a listing", async () => {
    ...
    const s = serverFor(f);
    expect(
      (await s.app.request("/api/models", { headers: LOOPBACK })).status,
    ).toBe(400);
    expect(
      (
        await s.app.request(`/api/models?path=${encodeURIComponent("/nope")}`, {
          headers: LOOPBACK,
        })
      ).status,
    ).toBe(404);
    ...
    const gone = await s.app.request(
      `/api/models?path=${encodeURIComponent(ROOT)}`,
      { headers: LOOPBACK },
    );
...
describe("reload, and the pass at startup (§6.5, §6.6)", () => {
  it("runs the incremental pass now and says whether anything moved", async () => {
    ...
    const quiet = await s.app.request("/api/reload", {
      method: "POST",
      headers: LOOPBACK,
    });
    ...
    const found = await s.app.request("/api/reload", {
      method: "POST",
      headers: LOOPBACK,
    });
    ...
    expect(
      (await listFlat(s)).entries.some((e) => e.name === "z/new.stl"),
    ).toBe(true);
  });
...
describe("emission fills what the layers lack, under a budget (§6.9)", () => {
  ...
  function stubFillIndex(
    ...
    return {
      ...
      posesAbout: (real: string): number =>
        bodies("/poses").filter((b) => b.includes(real)).length,
      /** `/under` asks that named this folder — one per preview derivation. */
      underAbout: (real: string): number =>
        bodies("/under").filter((b) => b.includes(real)).length,
  ...
  async function warmProbe(s: Server): Promise<void> {
    const res = await s.app.request("/api/semantic/status", {
      headers: LOOPBACK,
    });
    expect(res.status).toBe(200);
  }
   ...
   */
  const opensOf = (zip: string): number =>
    fs.opens.filter((path) => path === zip).length;
  ...
  it("carries pose and preview on the first sight, with no wave and no peek", async () => {
    ...
    expect(entryFor(listing, "a").preview?.map((e) => e.name)).toEqual([
      "bracket.stl",
      "part.stl",
    ]);
    expect(entryFor(listing, "z").preview?.map((e) => e.name)).toEqual([
      "bracket.stl",
    ]);
    ...
    for (const entry of listing.entries) {
      if (entry.kind === "model")
        expect(entry.pose, entry.path).not.toBeUndefined();
      if (entry.kind === "dir") {
  ...
  it("ships within the budget without the slow answer, and the next listing carries it", async () => {
    ...
    for (
      let i = 0;
      i < 50 && s.listings.layers.poseFor(`${ROOT}/loose.stl`) === undefined;
      i++
    ) {
      await settle(10);
    }
