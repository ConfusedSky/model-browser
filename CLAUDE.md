...
- `bun run dev:demo` - the same, under the shipped demo posture (`deploy/demo/config.json`
  via `MODEL_BROWSER_CONFIG`: every capability off, `intro` on). The file's `root` is the
  box's path, so the script overrides it with `MODEL_BROWSER_ROOT`, defaulting to this
  machine's copy of the box's corpus (`~/Documents/tests/test-models/miniatures/decimated`,
  the same layout default `deploy/demo/compose.yaml` takes for `CORPUS_DIR`); set
