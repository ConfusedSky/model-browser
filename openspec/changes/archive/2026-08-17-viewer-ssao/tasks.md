...
- [x] 2.1 Scale GTAO radius/thickness by staged `bounds.radius` and set the pass's scene clip box from the staged bounds box; tune constants visually on small and large fixtures, then freeze in a unit test (D3) — *tuned 2026-08-14 on all six e2e fixtures: `AO_SCALE` 1 → 1.5 (crevices now read at thumbnail size — the test cube's embossed digits resolve through AO alone); reach/thickness/falloff/samples kept at the defaults (0.15/0.3/1/16) — the reach doubles as the clip-box feather, and the edge gate stayed clean without widening it. Frozen in composer.test.ts* — **re-judge before archive**: the "AO dominates" reports that questioned these constants came from `Radroach_with_base.stl` and `Almenhier_body_32mm_unsupported.stl`, whose stored normals are rotated 90° off their geometry (see `stl-normals-from-winding` D4). AO amplifies exactly that corruption, so those models are evidence about the normals bug, not the fit. Leave `AO_RADIUS_R`/`AO_SCALE` as tuned until that change lands, then re-judge — including on a based miniature, which the six e2e fixtures do not cover. **Re-judged 2026-08-17, constants stand**: with honest normals (rig 6), AO on `Radroach_with_base` reads as contact and grounding — leg/ground junctions, the carapace undercut, and debris-plate seams — against a pasted-on look with AO off; base texture reads as form, not dirt, and the clip box leaves the base rim clean. The Almenhier body reads as a coherent sculpt likewise. The earlier dirt impression was the rotated-normals corruption, as stl-normals-from-winding D4 predicted
...
- [x] 4.3 ~~Remove the toggle before archive~~ — **promoted to a shipped preference instead (D6, 2026-08-17)**: AO off measured 17 → 56 fps on an iGPU orbit drag, so the pill stays as a per-profile performance control. Persistence added (localStorage, lighting-mode pattern, with test), scaffolding comments rewritten as shipped docs, spec delta re-worded from "not user-toggleable" to requiring the toggle, pill title states the perf purpose
