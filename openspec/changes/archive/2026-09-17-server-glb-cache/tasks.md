...
- [ ] 4.2 Manual: open an STL inside a zip; confirm it is served as GLB and that a second open does not re-read the zip (no `extractEntry` call — log or spy in dev). *Not exercisable on the demo corpus (0 zips); covered by the route unit test (`serves an STL zip entry as GLB, and a hit never reopens the zip`), which asserts one `MeshCache.write` across two requests.*
...
- **4.2 not exercisable here** — the demo corpus has 0 zips. Covered by the route unit test (`serves an STL zip entry as GLB, and a hit never reopens the zip`): one `MeshCache.write` across two requests, and `write` runs only after an extract + convert.
