...
- **Rim-shadow comparison toggle (verification scaffolding)**: a visible control in the viewer toggles whether the red/blue rim accents cast shadows in the live view — the rims stay lit in both states; only their shadow casting changes. Default off (the shipped recipe casts from the key alone), so the toggle temporarily adds the rims as casters and key-only vs. key+rim shadow casting can be compared. Live view only — thumbnails always render the shipped recipe, so the toggle never touches `RIG_VERSION` or the cache. It follows the lighting-mode pill precedent (client/src/viewer/lighting.ts), is not persisted, and is scaffolding: before this change is archived the comparison is judged and the toggle removed — or, if rim casting earns its keep, promoted by a follow-up change that specs it properly.
