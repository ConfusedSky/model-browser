...
**Resolved (2026-08-11): key-only wins. The scaffolding — flag module, pill, session hook, and the rim shadow-camera fitting — is removed; D2's single caster stands unchanged. Rim casting is not pursued as a follow-up.** The record below is the decision as it stood while the comparison ran.
...
- ~~Floor opacity and whether the contact shadow reads well on the dark app background at thumbnail size~~ **Answered: it reads well at 0.7; shipped enabled.** At the initial 0.35 a full-strength shadow composited only ~9/255 darker than the dark tile panel — invisible at thumbnail size (measured on the cached 512² PNGs: full shadow = alpha 89 of black, exactly 0.35·255). Raised to 0.7 and re-judged on all six e2e fixtures in both lighting modes (2026-08-14): thumbnails read grounded, the lightbox reads as product-shot grounding rather than dirt, and the bias/extent constants needed no compensating change. Disabled-by-default was not needed. Because rig 3 had already shipped to main and produced cached PNGs at 0.35 (this entry measured them), the opacity change bumped `RIG_VERSION` 3 → 4 per the recorded rule — pixel changes never skip the bump, even mid-change; the follow-up SSAO change's bump shifts to 4 → 5.
- ~~Key-only vs. key+rim shadow casting: which reads better?~~ **Answered: key only.** Judged on Enforcer_Robot_Dog.stl and bigmakerbottable4.stl in both lighting modes with identical poses (task 4.3). Rim casting did not read well: the two accents sit low (`y = 0.3`) and behind the subject, so their shadows travel far across the floor and run out of the `±2·radius` ortho extent sized for the key, leaving a hard rectangular frustum edge on the floor — most visible on the smaller model. Widening the extent would trade shadow-texel density across every light for a cue that added little beyond deeper ground contact. D2's single caster stands; no `RIG_VERSION` bump follows from this.
