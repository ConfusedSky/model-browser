...
> Added 2026-09-02 (review finding, archive blocker): `Grid`'s band observer
> now carries `onPeek`, so a folder's preview is requested when the tile comes
> within the prefetch band — screens before it is on screen — and this
> capability's *Folder tiles preview their contents* still said "when the tile
> is on screen". One sentence and one scenario body change; every other
> sentence and all nine scenarios carry unchanged. `search-cancellation`'s
> delta on this capability is ADD-only (*Concurrent and abandoned listing
> work*) — no title overlap.
