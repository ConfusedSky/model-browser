...
- Zip files browse like folders — contents listed from the central directory without extracting anything to disk; entries addressed via virtual paths (`foo.zip!/inner/part.stl`). Nested zips are listed but not enterable.
...
- Clicking a model tile (a press released without dragging) opens a lightbox expanded view with full orbit/zoom controls.
...
- New dependencies: hono, react, three, a zip library (e.g. fflate), vite, tailwind, vitest.
- Server-side cache directory (e.g. `~/.cache/model-browser/`) for thumbnails and camera states, size-capped and swept.
- Localhost-only: server reads arbitrary local paths, so the API is bound to 127.0.0.1 *and* guarded to the app's own origin (localhost binding alone does not stop other web pages). No user auth; not for hosted deployment.
