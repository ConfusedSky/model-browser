...
- **Search budget (backend)**: the search walk runs on `MODEL_BROWSER_SEARCH_BUDGET` (default 200k steps) instead of the browse walk's 20k — found in use when a deep match sat past the browse horizon of a large library and the search returned a false empty.
