...
The lightbox gains a compact control: three axis buttons (X/Y/Z) plus a flip toggle, showing the model's current spindle. Changing it re-frames the view to the default three-quarter view *for that spindle* (the old camera state is meaningless in the new frame) and persists axis + camera + re-snapshotted thumbnail on the spot. The in-grid orbit overlay has no control — it reads the model's stored axis; the lightbox is where deliberate per-model configuration happens.
