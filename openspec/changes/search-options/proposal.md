...
- Ordering: **after `search-matches-folder-names`**, whose predicate this gates. Also coordinate with `listing-tree-cache`: any option that reaches the server changes what a cached walk means, so it must be part of that cache's key.
