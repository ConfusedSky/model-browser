...
- [ ] 1.3 Bounding and ordering (D2/D3): hard walk budget on directories visited and models scanned; collect model metadata to that budget, sort models by basename with the full relative path as tiebreak, return the first 500; set `truncated` when the cap **or** the budget dropped anything; budget reads the `MODEL_BROWSER_FLAT_BUDGET` env override (mirroring `MODEL_BROWSER_CACHE`) so tests can reach it with small fixtures. Do NOT pass the combined array through `sortEntries` — its model comparison is `localeCompare` on `name`, which for flat entries is the relative path
...
- [ ] 2.3 Truncation notice above the grid when `truncated` is set, worded off the returned model count — never a hardcoded 500, since budget truncation can return fewer (D3/D4)
