...
**View equality is one function everywhere it appears:** `sameView(a, b) = serializeView(a) === serializeView(b)` — the comparison urlState.ts:111-113 already makes, justified by its comment at :103-110. It is what acceptance's `forView` match means (R2), what the `commitUrl` dedupe compares (urlState.ts:125), and what the stood-in predicate `!sameView(result.forView, view)` means (R6). Never reference or field-wise comparison: fetchless patches mint new objects, so identity comparison would misfire on the first `setKinds`. Two consequences recorded from commit 1: (a) the restore compare is not this one at all but its sibling over the *request*, `sameQuestion(a, b) = requestOf(a) equals requestOf(b)` — the question the server answers, as against the URL the view names — so a history entry that asks the same question patches its request-irrelevant fields (`model`, `kinds`, `mode`, the `flat` toggle under a committed query, `tuning` under a name search) rather than re-asking. Which model is open says nothing about which entries the view contains (R7's two truths), and neither does the kind option, which selects among entries already landed; the model-only reading (`sameListing`, kept for the tuning debounce guard below) was the same insight applied to one field, and re-fetched a whole directory on a Back across a kind change (reported 2026-08-21). This replaces today's popstate `changed` list at App.tsx:764-770, whose omission of tuning is a finding; (b) `dest`'s "in flight if there is one, else asserted" rule is the general one (`liveView`), read for `path` by the PathBar and for every option by the controls — a stand-in is skipped, because it is not the question.
