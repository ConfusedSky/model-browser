...
On tile hover (after ~120ms linger debounce), fetch + parse the mesh into a client-side LRU so mousedown-orbit is instant. Concurrency cap (~2 parses in flight) prevents hover-storms. LRU budget is measured in **bytes** (~300–500MB), not entry count — with 1–100MB files, "last N entries" could mean gigabytes. Mousedown before warm completes → brief spinner in the overlay (no worse than the unwarmed baseline).
