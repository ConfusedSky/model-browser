...
- [ ] 1.4 `resolve(libPath)` → `{ fsPath, entry? }`: refuse unless it starts with `/`;
      `posix.normalize`; `parseVPath` on the normalized form; `join(top, fsHalf)`;
      `realpath` and require `=== realTop || startsWith(realTop + sep)`. Refusals are
...
- [ ] 2.1 `listing.ts`: `listDir`/`listFlat` take a library path, resolve through 1.4,
      and emit every `DirEntry.path` as a library path (`libPathOf` at `listFsDir`'s
      seam and `joinVPath` for archives); `walkFsLevel` skips a subdirectory whose
...
- [ ] 3.2 Migration on first `ready`: scan `<base>/*.json` (legacy flat layout only —
      ...
      sidecar. Entries elsewhere are left. Idempotent: a second start finds nothing to
      move
...
- [ ] 4.1 `semantic.ts`: `scopeWithin` takes a library path and resolves through 1.4;
      ...
      containment guard stays; `modelEntryAt` receives the library path to emit
...
- [ ] 5.4 The `unconfigured` and `missing` states render in the path bar's error line
      ...
      view
- [ ] 5.5 Client tests: `serializeView`/`parseUrl` round-trip `/`, `/Kit`,
...
- [ ] 6.3 `listing-tree-cache/design.md`: add the per-library keying and directory
      (`id` + library path under `<base>/<id>/`) before that change is applied
