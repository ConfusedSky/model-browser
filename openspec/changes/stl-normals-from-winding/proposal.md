...
- **STL parsing ignores stored facet normals**: `parseModel` discards the normal attribute `STLLoader` builds from the file and recomputes flat facet normals from triangle winding via `computeVertexNormals()` — winding is the authoritative orientation data per the STL spec, and it is what slicers actually print from. Healthy files (stored ≡ winding) render identically; convention-mismatched files become correct.
- **Thumbnail refresh via `RIG_VERSION`**: recomputed normals change rendered pixels (for affected models materially; for healthy models only to float-precision), so the shared constant bumps 6 after viewer-ssao's 5, riding the existing lazy re-render sweep.
...
- `client/test/` — unit coverage parsing a crafted binary STL whose stored normals are winding rotated 90° about X, asserting the parsed geometry's normals match winding, not the file.
