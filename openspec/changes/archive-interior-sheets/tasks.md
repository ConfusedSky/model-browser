...
- [ ] 0.1 `search-cancellation` shares `server/src/listing.ts` and states in its
      proposal that "`takeStep` has only two call sites" and that the peek is
      exempt by construction. Task 1.5 adds a third. Land in either order, but
      whichever lands second re-checks that claim and that change's peek-exemption
      cell against the interior walk. Note the dependency in both changes.
- [ ] 0.2 `hover-prefetch-listings` warms `/api/dir` for hovered **zip** tiles.
...
- [ ] 1.1 Replace `peek`'s `if (entry !== undefined) return []` (server/src/listing.ts)
...
- [ ] 1.2 Give the branch the listing's refusals rather than a bare `[]` (D10):
...
- [ ] 1.3 Add the interior level walk: filter `listZipEntries`' names by the
...
- [ ] 1.4 Carry `zipStat.mtimeMs` on every interior find, as `listZipDir` and
...
- [ ] 1.5 Charge `PEEK_BUDGET` **one step per distinct immediate child** — each
...
- [ ] 1.6 Thread an optional `ZipDirCache` through `peek` to `listZipEntries`,
      the way `gatherFlat` threads `walk.zips` (D3).

## 2. Server: the shared archive read, routes, and invalidation

- [ ] 2.1 Fold `listZipDir` onto the layer (D3): it takes a `ZipDirCache`,
...
- [ ] 2.2 Thread the cache to where `peek` is actually called: neither route calls
...
- [ ] 2.3 Re-derive a stale interior sheet at emission (D9): the held cells carry
...
- [ ] 2.4 Update `selfAndAncestors`' comment in server/src/layers.ts — "A preview
      is never derived inside an archive today" becomes false with 1.1.

## 3. Server tests

- [ ] 3.1 Rewrite `server/test/peek.test.ts`'s "previews nothing for a path
...
- [ ] 3.2 Keep and re-assert the two neighbours that stay true: a zip **root**
      previews nothing, and an archive met on the way during a filesystem walk is
      not entered.
- [ ] 3.3 Cover an interior whose own level holds only subdirectories (descends),
...
- [ ] 3.4 Cover 1.2's refusals as refusals, not as empty sheets — `/somedir!/x`,
...
- [ ] 3.5 Assert an interior find's entry equals the listing's for the same model,
      mtime included (1.4), so the shared-thumbnail requirement is pinned rather
      than assumed.
- [ ] 3.6 Assert the archive is read once for a listing plus its tiles, through a
      ...
      **zero** reads and 3.8's revert also shows zero. Use the harness in
      ...
      since D3 claims it.
- [ ] 3.7 Cover 2.3: a sheet held against an older archive mtime is re-derived at
      emission rather than served.
- [ ] 3.8 Falsify every cell before trusting it — revert 1.1's branch (and 2.3's
      check for 3.7) and confirm each new test fails for the reason it names, not
      merely fails.

## 4. Verify against the running app

- [ ] 4.1 `bun run typecheck` and `bun run test` clean across workspaces.
- [ ] 4.2 Live check on the real library, from a restarted server: `/api/peek`
...
- [ ] 4.3 Browse 5173 (not 3177 — a stale `client/dist` would serve an old
      bundle) into a zipped kit and confirm the folder tiles fill. Judge the
      pixels; this line is not done when the code lands.
- [ ] 4.4 Land the cost probe as `scripts/zip-tail-cost.ts` — the Bun script
