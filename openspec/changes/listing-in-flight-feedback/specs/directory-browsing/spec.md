...
While a directory listing request is in flight, the client SHALL acknowledge it visibly: if the newest request has not resolved within a short reveal delay, the grid SHALL be replaced by a skeleton of placeholder tiles until that request lands. A request that resolves within the delay SHALL never show the skeleton. While the skeleton is shown, the previous listing's tiles SHALL NOT be interactive, but the path bar, parent navigation, and the flat toggle SHALL remain usable. The in-flight indication SHALL follow the newest request only: a superseded request's landing SHALL neither dismiss nor re-trigger it, and a failed request SHALL clear it and surface the error over the listing the user was already viewing.
...
- **WHEN** the user navigates and the listing resolves within the reveal delay
