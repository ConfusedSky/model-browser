...
Two `DirectionalLight`s added in `makeScene` beside the current three: red positioned at rig-space left and slightly behind the subject (negative x, negative z), blue mirrored at the right, both aimed at the origin like the key and fill (default target). Because the rig's quaternion is the single orientation authority, the accents inherit the axis/camera split, the tween slerp, and the drag-cancel snap with zero new code — in `camera` mode rig-space −x̂ is screen-left by construction, and in `axis` mode it is the frame's −a direction. Intensities are chosen as accents (order 0.3–0.6) so they tint edges without shifting overall exposure; exact values and heights are tuned visually at apply time and then frozen in the unit test.
