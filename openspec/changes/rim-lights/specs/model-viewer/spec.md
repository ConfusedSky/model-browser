...
The rig SHALL additionally carry two colored rim accents — red at the rig's left, blue at its right, both modest in intensity and placed slightly behind the subject — present in both modes. Because they are part of the rig, they SHALL follow its orientation everywhere the rig does: in `camera` mode the red accent stays at screen-left while the model orbits, in `axis` mode both accents stay fixed in the spindle frame so they turn with the model's own sides, and during the axis tween they rotate with the rig.
