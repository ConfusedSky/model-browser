...
- [ ] 1.4 Index-width test: `uint16` holds indices 0…65535, so a mesh with exactly 65 536 welded vertices gets a `uint16` index accessor and one with 65 537 gets `uint32`. Verify both cells pass, so the branch is `count <= 65536`, not `<`.
