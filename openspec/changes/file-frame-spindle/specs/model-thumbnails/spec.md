...
- **THEN** it keeps no axis — a stored axis would withhold an index orientation the entry never suppressed — and the migration tool only stamps the frame label; it renders about the format's default, whose frame is the frame that +Y had, so the camera is interpreted unchanged and the re-rendered thumbnail shows the same view it did before
...
- **WHEN** a cache entry written before axes were file axes holds a camera on an OBJ at spindle `z`
