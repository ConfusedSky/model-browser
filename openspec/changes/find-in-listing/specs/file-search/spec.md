...
> modifies both of these requirements and is a hard prerequisite (tasks.md).
>
> Two sentences that prerequisite just added are deliberately dropped, so the next reader
> diffing against it does not read them as accidents. From *Live name filter*: "the same
> string a deep search matches, so a folder fragment narrows the grid here and finds
> results there alike" — true while one predicate served both, and not something to promise
> once the filter and the query are separate controls over possibly different corpora. From
> *Deep name search*: "distinct from typing, which only filters" — typing in the search
> input no longer filters, which is the whole change, and leaving the clause would ship a
> spec that contradicts itself between this change's archive and the next one's.
