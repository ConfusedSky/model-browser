...
- [The D6 AO toggle outlives the change if the removal task is skipped] → task 4.3 removes it before archive is reachable; if it is instead kept deliberately, that decision must become its own change with a spec delta.
