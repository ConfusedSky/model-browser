...
### D6: A live-view AO toggle — built as verification scaffolding, promoted to a shipped preference

To let the AO recipe be judged against a no-AO render in place, the viewer gains a pill ("ssao", lighting-pill precedent) that turns GTAO off in the live view. Mechanism mirrors viewer-shadows' rim toggle: an in-memory flag module (`viewer/aoToggle.ts`, default ON — the shipped recipe includes AO), consulted by `ViewerSession.render` alone and forwarded to the live chain per render as an `ao` argument that flips the GTAO pass's `enabled` (composers skip disabled passes; the RenderPass → OutputPass swap is unchanged). `renderThumbnail` never passes the argument, so thumbnails, `RIG_VERSION`, and the cache stay on the shipped recipe; while AO is off the live view intentionally diverges from cached thumbnails, knowingly suspended for the comparison. **Promoted (2026-08-17):** GTAO is bandwidth-bound and integrated GPUs feel it first — measured 17 → 56 fps on an orbit drag with occlusion off (Radeon 780M, supersampled lightbox) — so on dual-GPU machines where the everyday browser runs on the iGPU, the toggle is a real performance preference, not just a comparison instrument. It now persists per browser profile via localStorage (lighting-mode pattern), which composes with the dual-browser workflow: an iGPU profile can keep AO off while a dGPU profile keeps it on. The spec delta's requirement changed from "not user-toggleable" to requiring the toggle; thumbnails still always render the shipped recipe, so the cache and `RIG_VERSION` never see the preference and no bump is needed. If no-AO were ever preferred as the *default*, that would be its own change with a version bump.

### D7: The live view renders above device resolution (added post-implementation)

A thumbnail is a 512² render displayed in a ~176 px tile — supersampled by construction — while the live canvas rendered 1:1 with device pixels, making the *interactive* view the more aliased of the two at handoff. The residue is shading aliasing (high-frequency normals on dense sculpts, amplified by AO), which MSAA cannot touch: multisampling resolves geometric edge coverage, not the shader result within a fragment. `liveRenderSize` (viewer/renderSize.ts) therefore sizes the live backing store at 1.5× device resolution, clamped to a 6M-pixel ceiling that caps the supersample without ever dropping below device resolution (that would trade aliasing for blur); the canvas keeps its CSS box (`style` pinned to 100%, `setSize(…, false)`), so the browser downsamples. Constants frozen in renderSize.test.ts. Thumbnails are untouched — `RIG_VERSION` versions the thumbnail recipe and does not bump — and handoff parity improves: a tile-sized overlay now renders ~528² against the thumbnail's 512², nearly matching sample density (verified: handoff clips remain pixel-indistinguishable).
...
- [The D6 AO toggle was scaffolding slated for removal] → resolved by keeping it deliberately: D6 records the promotion and the spec delta now requires the toggle, so the archived spec and the shipped UI agree.
