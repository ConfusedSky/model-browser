...
- [x] 1.1 The report type in `shared/types.ts` (`thumbWrites: boolean` to start) and a
...
- [x] 1.2 `GET /api/features` in `app.ts` answering it, added to `UNGATED` beside
      `/api/library` and `/api/apps` (design D2)
- [x] 1.3 Server tests: answers all-on by default; answers an injected variant;
      answers while unconfigured (the UNGATED cell — `/api/apps` has the precedent)

## 2. Client

- [x] 2.1 `ApiClient.features()` (`jsonOrThrow`, like `apps()`); `App` fetches it in a
...
- [x] 2.2 Client tests: harness gains a `features` mock defaulting to a known all-on
...
> 1.1–2.2 landed 2026-09-02 in e431360 (opus worker, coordinator line-review): suites
> 652 client / 494 server green, typecheck 0 standalone (shared-tree failures were the
> sibling change's, attributed by pristine-archive cross-check); eight falsifications
> run and restored, incl. the byte-identity cell (harness binding removed → all 7
