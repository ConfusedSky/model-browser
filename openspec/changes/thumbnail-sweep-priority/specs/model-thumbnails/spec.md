...
> added later the same day (user review — see design.md D4/D6), and the whole
> change re-verified at HEAD 2026-09-02 by an opus review (no citation drift;
> that review's fold-in also added the second MODIFIED block below). MODIFIES
> *Client-side thumbnail rendering* and — for one clause parking would otherwise
> contradict — *Recipe-labelled thumbnails*; no other active change touches this
...
> `library-overrides` archived 2026-09-01). The first MODIFIED block carries
...
- **THEN** the render is kept and completes behind all on-screen work, filing the cached image — and if the mesh has been evicted by the time its turn comes, the work is parked at that point without the model being read again, unless its tile is no longer reported far by then, in which case it is read and rendered like any tile on or near the screen
