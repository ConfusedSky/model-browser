...
> Rebased 2026-09-01 against main `62f9f2d`. MODIFIES *Client-side thumbnail
> rendering* only; no other active change touches this capability
> (`adaptive-ao-default` and `library-overrides` modify `model-viewer`,
> `directory-browsing` and `library-overrides`). The MODIFIED block below carries
...
- **THEN** every parked model's cache is consulted at once — one already held under the new setting is shown straight away — and only a model the cache does not hold under the new setting stays parked until its tile returns
