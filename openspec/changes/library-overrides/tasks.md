...
- [x] 1.1 `server/src/overrides.ts`: the store module — `loadOverrides(top)` reading
...
- [x] 1.2 Types in `shared/types.ts`: the per-key fields (`name`, `credits {author,
      ...
      is declared
- [x] 1.3 Lifetime: the store is loaded when the library first resolves `ready` and held
...
- [x] 1.4 `app.ts` `GET /api/overrides?path`: `path` required (400 `path is required`,
...
- [x] 1.5 Server tests (`server/test/overrides.test.ts`, conventions per
...
- [x] 1.6 Display names ride the listing (D7): at emission, each entry's exact library
...
- [x] 3.1 `scripts/gen-overrides.ts` (run from repo root with `bun run`, corpus side):
...
- [x] 3.2 Generator test (or a self-checking dry-run mode): rerun over a store carrying
