...
task before any row was measured); and while the furthest point the layout can reach — the target, or
the end of the layout's total if the target lies past it — is beyond the *rendered* scroll extent,
it waits rather than writes: the body re-renders to the grid's new height one render after the
measurements that grew it, and a write the browser clamps against the stale extent moves nothing
it can keep (follow-up 7.6, 2026-09-25); while a measurement has moved the row since this commit drew it, it waits again;
