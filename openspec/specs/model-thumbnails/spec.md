...
#### Scenario: The source's opinion of a model changes
- **WHEN** a model the source would frame is displayed, its cached thumbnail carrying the orientation key it was drawn under, and the source now holds a different orientation for it
- **THEN** the thumbnail is re-rendered under the new orientation and records its key, keeping the old image until the replacement lands

#### Scenario: A render labelled before the key existed is re-rendered once
- **WHEN** a model the source would frame is displayed, its cached thumbnail carrying the current mapping version and no orientation key
...
- **THEN** the thumbnail is a hit and nothing is rendered or uploaded
