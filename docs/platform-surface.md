...
|---|---|---|---|
| `default(mime)` | `gio mime <mime>`, parsed | file-extension registry (`HKCU\...\FileExts`, `assoc`/`ftype`) | LaunchServices; no stock CLI — third-party `duti` or a tiny helper |
| `associations(mime)` | `gio mime <mime>`, parsed | `OpenWithList`/`OpenWithProgids` registry keys | LaunchServices (`LSCopyApplicationURLsForContentType`) via helper |
