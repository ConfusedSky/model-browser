...
function lift(
  table: Record<OrbitAxis, FrameTriples>,
): Record<OrbitAxis, SpindleFrame> {
  return Object.fromEntries(
    (Object.entries(table) as [OrbitAxis, FrameTriples][]).map(
      ([axis, { s, a, b }]) => [
        axis,
        {
          s: new THREE.Vector3(...s),
...
export function stateTarget(state: CameraState, bounds: Bounds): THREE.Vector3 {
  return new THREE.Vector3(...state.target)
    .multiplyScalar(bounds.radius)
    .add(bounds.center);
}
