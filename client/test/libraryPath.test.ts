...
describe("expandLibraryPath", () => {
  it("joins the library top onto a library path", () => {
    ...
    expect(
      expandLibraryPath("/run/media/masa/STL Library", "/Loot/x.stl"),
    ).toBe("/run/media/masa/STL Library/Loot/x.stl");
  });
  ...
  it("keeps the `!/` notation, entry name untouched", () => {
    ...
    expect(
      expandLibraryPath("/lib", "/Kit/parts.zip!/inner dir/part v2.stl"),
    ).toBe("/lib/Kit/parts.zip!/inner dir/part v2.stl");
    // The archive itself, with no entry named, joins like any other path.
    expect(expandLibraryPath("/lib", "/Kit/parts.zip")).toBe(
      "/lib/Kit/parts.zip",
    );
  });
  ...
  it("does not double the separator when the top ends in one", () => {
    expect(expandLibraryPath("/lib/", "/Kit/a.stl")).toBe("/lib/Kit/a.stl");
    expect(expandLibraryPath("/lib/", "/")).toBe("/lib");
    expect(expandLibraryPath("/lib/", "/Kit/a.zip!/x.stl")).toBe(
      "/lib/Kit/a.zip!/x.stl",
    );
  });
  ...
  it("hands back the library path unchanged when there is no top", () => {
    ...
    expect(expandLibraryPath(null, "/Kit/a.zip!/x.stl")).toBe(
      "/Kit/a.zip!/x.stl",
    );
