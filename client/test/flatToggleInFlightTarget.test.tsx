...
const AT_A_NESTED: DirListing = {
  path: "/models/a",
  entries: [dirEntry("/models/a/sub")],
};
...
const AT_ROOT_NESTED: DirListing = {
  path: "/models",
  entries: [dirEntry("/models/a")],
};
...
describe("flat toggle follows the in-flight navigation target", () => {
  it("untoggling flat while an ↑ navigation is in flight requests the destination and lands there nested", async () => {
    ...
    listDir.mockImplementation((target: string, opts?: { flat?: boolean }) => {
      ...
      if (target === "/models" && !isFlat)
        return Promise.resolve(AT_ROOT_NESTED);
      ...
      if (target === "/models/a" && !isFlat)
        return Promise.resolve(AT_A_NESTED);
      return Promise.reject(
        new Error(`unexpected listDir(${target}, flat=${isFlat})`),
      );
    });
  ...
  it("toggling after the newest navigation failed re-requests the committed path", async () => {
    listDir.mockImplementation((target: string, opts?: { flat?: boolean }) => {
      ...
      if (target === "/models/a" && opts?.flat === true)
        return Promise.resolve(AT_A_FLAT);
      return Promise.reject(
