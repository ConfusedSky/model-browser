...
describe("FILE_FRAMES", () => {
  it("is design D3's table, all six entries", () => {
    for (const axis of AXES)
      expect(FILE_FRAMES[axis], axis).toEqual(D3_TABLE[axis]);
  });
  ...
  it("is keyed by the axis its own spindle vector names", () => {
    for (const axis of AXES)
      expect(axisOfTriple(FILE_FRAMES[axis].s)).toBe(axis);
  });
...
describe("unbake and axisOfTriple", () => {
  ...
  it("axisOfTriple throws on anything but a unit axis vector", () => {
    expect(() => axisOfTriple([0, 0.999, 0.02])).toThrow(
      /not a unit axis vector/,
    );
...
describe("swapOffset", () => {
  it("is x −90°, -x +90°, z +90°, -z −90°, y 0, -y 0", () => {
    const want: Record<OrbitAxis, number> = {
      x: -90,
      "-x": 90,
      z: 90,
      "-z": -90,
    ...
    for (const axis of AXES)
      expect(deg(swapOffset(axis)), axis).toBeCloseTo(want[axis], 9);
  });
    ...
    for (const axis of AXES) {
      ...
      for (const az of [-2.5, -0.7, 0, 0.3, 1.1, 2.9]) {
        for (const el of [-0.5, 0, 0.4]) {
          ...
          expect(
            angleDelta(now.az, az + swapOffset(axis)),
            `${axis} @${az}/${el}`,
          ).toBeCloseTo(0, 9);
