...
import { indexFingerprint, manifestFor, POSE_CACHE_FILE, RUN_PARAMS_FILE, writeManifest } from '../../scripts/bake-demo'
...
it('exits 0 silently for a manifest baked at the live RIG_VERSION and POSE_VERSION', async () => {
  ...
  const manifest = await writeManifest(
    ...
    manifestFor({
      ...
      client: { dirty: false },
      recipe: { rig: RIG_VERSION, poseVersion: POSE_VERSION, lighting: 'camera', size: 256 },
      library: { id: 'lib', root: '/library' },
      models: 1,
      verify: { renders: { ao: 1, noao: 1 }, posed: 1, unposed: 0 },
      passes: { ao: { rendered: 1, elapsed: 1 }, noao: { rendered: 1, elapsed: 1 } },
      index: {
        collectionRoot: '/',
