...
// for.
export const models = vi
  .fn()
  .mockResolvedValue({ path: "/", entries: [], complete: true });
  ...
  return {
    ...
    getRenderer: () =>
      ({
        ...
        typeof import("../src/three/renderer").getRenderer
      >,
...
export const click = (el: HTMLElement): Promise<void> =>
  act(async () => el.click());
...
export function upButton(): HTMLButtonElement {
  return container.querySelector<HTMLButtonElement>(
    'button[aria-label="Parent directory"]',
  )!;
}
...
export function findInput(): HTMLInputElement | null {
  return container.querySelector<HTMLInputElement>(
    'input[aria-label="Narrow these by name"]',
  );
}
...
export function aoPill(): HTMLButtonElement {
  return container.querySelector<HTMLButtonElement>(
    'button[title^="Ambient occlusion"]',
  )!;
}
export function deepButton(): HTMLButtonElement {
  ...
  return container.querySelector<HTMLButtonElement>(
    'button[title^="Search this folder"]',
  )!;
}
