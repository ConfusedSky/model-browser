...
const paths = (entries: { entry: DirEntry }[]): string[] =>
...
describe("what a generate derivation keeps", () => {
  it("keeps every model the current build would draw differently, and drops the ones it would not", async () => {
    const h = harness(
      listing([
        ...
        model("stale", {
          thumb: thumb({
            ao: { state: "stale", lighting: THUMB_LIGHTING, rig: RIG_VERSION },
          }),
        }),
        model("wrong-rig", {
          thumb: thumb({ ao: currentRender({ rig: RIG_VERSION + 1 }) }),
        }),
        // 'axis' is the retired lighting label and reads as stale — never as
        // "the current mode" (client/test/CLAUDE.md).
        model("old-lighting", {
          thumb: thumb({ ao: currentRender({ lighting: "axis" }) }),
        }),
        ...
        model("owned", {
          ...
          thumb: thumb({
            framed: true,
            camera: { az: 1, el: 0.2, distR: 3, target: [0, 0, 0] },
          }),
        }),
      ]),
    );
  ...
  it("reads the variant for the occlusion setting in force, not the other one", async () => {
    const entries = [
      model("m", {
        thumb: thumb({ ao: currentRender(), noao: { state: "miss" } }),
      }),
    ];
    ...
    expect(paths((await off.jobs.derive("generate", SCOPE)).entries)).toEqual([
      "/kit/m.stl",
    ]);
  });
  ...
  it("snapshots each kept entry’s generation, and calls a model the server knows nothing about zero", async () => {
    const h = harness(
      listing([
        model("a", { thumb: thumb({ gen: 9, ao: { state: "miss" } }) }),
        model("b"),
      ]),
    );
  ...
  it("keeps exactly the models whose framing a reset would change", async () => {
    const h = harness(
      listing([
        ...
        model("axis-only-poseless", {
          pose: POSE_OFF_AXIS,
          thumb: thumb({ framed: true, axis: "z" }),
        }),
        // The same axis where a usable pose replaces it: given up, counted.
        model("axis-only-posed", {
          pose: POSE,
          thumb: thumb({ framed: true, axis: "z" }),
        }),
...
describe("the job’s own orientation wave", () => {
  it("asks about exactly the models the enumeration had no pose for", async () => {
    ...
    expect(h.posesFor).toHaveBeenCalledWith([
      "/kit/unknown-a.stl",
      "/kit/unknown-b.stl",
    ]);
  ...
  it("does not ask at all when the enumeration answered for every model", async () => {
    const h = harness(
      listing([model("a", { pose: POSE }), model("b", { pose: POSE })]),
    );
...
describe("the generate run", () => {
  ...
  it("forwards the snapshotted generation on every write", async () => {
    const h = harness(
      listing([
        model("a", { thumb: thumb({ gen: 11, ao: { state: "miss" } }) }),
      ]),
    );
    ...
    expect(h.putThumb.mock.calls[0]![0]).toMatchObject({
      path: "/kit/a.stl",
      ifGen: 11,
    });
  });
...
describe("the reset run", () => {
  ...
  it("restarts an on-screen tile after each accepted write, and after no refused one", async () => {
    ...
    h.putThumb
      ...
      .mockRejectedValueOnce(new HttpError(412, "moved"));
...
describe("one job at a time, cancelled at any instant", () => {
  it("answers busy to a second launch, starts nothing, and brings the chip back", async () => {
    ...
    expect(h.jobs.launch("reset", { path: "/other", label: "other" })).toBe(
      "busy",
    );
    await settle();
  ...
  it("derives the remainder when it is launched again", async () => {
    ...
    expect(h.push.mock.calls.map((c) => c[1])).toEqual([
      "/kit/b.stl",
      "/kit/c.stl",
    ]);
    expect(h.jobs.state).toMatchObject({ phase: "done", total: 2, done: 2 });
      ...
      Promise.resolve(
        path === "/a"
          ...
          : listing([
              model("b1", {
                pose: POSE,
                thumb: thumb({ framed: true, camera: CAMERA }),
              }),
            ]),
      ),
    );
...
describe("counting a scope", () => {
  it("answers both counts from one enumeration and one wave", async () => {
    ...
    expect(await h.jobs.count(SCOPE)).toEqual({
      generate: 2,
      reset: 2,
      incomplete: true,
    });
...
describe("what a count asks the index about", () => {
  it("a count and a reset ask nothing; a generate derivation waves over all the unknown", async () => {
    ...
    const h = harness(
      listing([
        model("unposed-plain"),
        model("unposed-axis-only", {
          thumb: thumb({ framed: true, axis: "z" }),
        }),
        model("unposed-camera", {
          thumb: thumb({
            framed: true,
            camera: { az: 1, el: 0.2, distR: 3, target: [0, 0, 0] },
          }),
        }),
        model("posed-axis-only", {
          pose: POSE,
          thumb: thumb({ framed: true, axis: "z" }),
        }),
        model("camera-and-axis", {
          thumb: thumb({
            framed: true,
            axis: "z",
            camera: { az: 1, el: 0.2, distR: 3, target: [0, 0, 0] },
...
describe("what a job reports it wrote", () => {
  ...
  it("settles only when the in-flight entry has landed, cancelled or not", async () => {
    ...
    h.putThumb.mockImplementationOnce(
      () =>
        new Promise((r) => {
          land = () => r({ gen: 2 });
        }),
...
describe("what the chip can say about a wait", () => {
  ...
  it("never reports a reset as waiting: its writes go nowhere near the queue", async () => {
    const h = harness(
      listing([model("a", { thumb: thumb({ framed: true, camera: CAMERA }) })]),
    );
    h.queue.suspend();
    h.jobs.launch("reset", SCOPE);
    await settle();
