...
describe("the shifted secondary press", () => {
  ...
  it("is left to the browser on a folder tile and on a model tile; unshifted still raises", async () => {
    expect(await secondaryPress(tile("Alpha"), 120, 140, { shift: true })).toBe(
      false,
    );
    expect(menu()).toBeNull();

    expect(
      await secondaryPress(tile("widget.stl"), 120, 140, { shift: true }),
    ).toBe(false);
    expect(menu()).toBeNull();
  ...
  it("closes an open menu on the full shifted press, as the user makes it", async () => {
    await secondaryPress(tile("widget.stl"));
    expect(menu()).not.toBeNull();
    expect(await secondaryPress(tile("Alpha"), 40, 40, { shift: true })).toBe(
      false,
    );
    expect(menu()).toBeNull();
  });
...
describe("raising the menu", () => {
  ...
  it("is reachable from the keyboard and returns focus to the tile", async () => {
    ...
    expect((document.activeElement as HTMLElement).dataset.command).toBe(
      "reveal",
    );
