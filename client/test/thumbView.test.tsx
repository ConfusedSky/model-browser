...
describe("the tile image’s placeholder", () => {
  ...
  it("a folder sheet’s cell follows the same rule, keyed on its own entry", async () => {
    ...
    const cellUrl =
      "/api/thumb/image?path=%2Fmodels%2Fa%2Fm0.stl&mtime=1&gen=5";
    ...
    const cellImg = (): HTMLImageElement =>
      host.querySelector("[data-preview-cell] img")!;
    ...
    await renderSheet(
      {
        status: "ready",
        url: "/api/thumb/image?path=%2Fmodels%2Fa%2Fm0.stl&mtime=2&gen=7",
        gen: 7,
      },
      { ...cell, mtime: 2 },
    );
  ...
  it("starts over for a same-path entry at a new mtime — a different render, not a later URL", async () => {
    ...
    await renderTile(
      {
        status: "ready",
        url: "/api/thumb/image?path=%2Fmodels%2Fone.stl&mtime=2&gen=7",
        gen: 7,
      },
      { ...ENTRY, mtime: 2 },
    );
