...
import { findInput, model, mountApp, openFind, tiles, unmountApp } from './appHarness'
...
async function fireArrow(key: string, opts: { alt?: boolean } = {}): Promise<KeyboardEvent> {
...
describe('grid arrow-key focus movement', () => {
  ...
  it('columnCount counts the leading top-row tiles', () => {
    ...
    const fake = rects.map(
      (top) => ({ getBoundingClientRect: () => ({ top }) }) as unknown as HTMLElement,
    )
