...
const SEARCH: DirListing = {
  path: "/models",
  entries: [model("k03/found.stl")],
};
    ...
    (target: string, opts?: { flat?: boolean; q?: string }) => {
      ...
      if (target === "/models") {
        return Promise.resolve(
          structuredClone(opts?.flat === true ? FLAT_PARENT : PARENT),
        );
      }
 ...
 */
function holdListing(
  path: string,
  count = 1,
): { release: (listing: DirListing) => Promise<void> } {
...
async function openLightbox(): Promise<void> {
  const modelTile = main().querySelector<HTMLElement>(
...
const markedTile = (): string | undefined =>
  container
    .querySelector(".animate-reveal-mark")
    ?.getAttribute("data-entry-tile") ?? undefined;
