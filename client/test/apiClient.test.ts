...
describe("HttpApiClient contract", () => {
  it("listDir encodes the path (spaces, !/ virtual separator)", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ path: "", entries: [] }));
  ...
  it("listDir emits flat=true only when asked", async () => {
    const fetchFn = vi.fn((_url: string) =>
      Promise.resolve(jsonResponse({ path: "", entries: [] })),
    );
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    await api.listDir("/models", { flat: true });
    expect(fetchFn).toHaveBeenCalledWith(
      `/api/dir?path=${encodeURIComponent("/models")}&flat=true`,
      {
        signal: undefined,
      },
    );
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      `/api/dir?path=${encodeURIComponent("/models")}`,
      {
        signal: undefined,
      },
    );
    expect(
      fetchFn.mock.calls.filter(([url]) => url.includes("flat")),
    ).toHaveLength(1);
  });
  ...
  it("listDir hands its signal to fetch — a superseded walk is cancellable", async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      expect.stringContaining("/api/dir?path="),
      {
  ...
  it("semanticPoses asks about one directory, escaped", async () => {
    ...
    await expect(api.semanticPoses("/my models/kit")).resolves.toEqual({
      poses: {},
    });
    expect(fetchFn).toHaveBeenCalledWith(
  ...
  it("semanticPosesFor posts the models it was handed, in one request", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ poses: { "/a/x.stl": 1 } }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    ...
    await expect(
      api.semanticPosesFor(["/a/x.stl", "/b/y.stl"]),
    ).resolves.toEqual({
      poses: { "/a/x.stl": 1 },
    });
  ...
  it("semanticPosesFor chunks a listing past the route bound and merges the answers", async () => {
    ...
    const fetchFn = vi
      ...
      .mockResolvedValueOnce(
        jsonResponse({ poses: { [`/models/m${POSES_MAX}.stl`]: 2 } }),
      );
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    ...
    const sent = fetchFn.mock.calls.map(
      (c) =>
        (JSON.parse((c[1] as { body: string }).body) as { paths: string[] })
          .paths,
    );
  ...
  it("semanticPoses raises the server failure rather than swallowing it", async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ error: "no such directory" }, 404));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    await expect(api.semanticPoses("/gone")).rejects.toThrow(
      "no such directory",
    );
  });
  ...
  it("throws HttpError with the server message on failure", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ error: "nested zips are unsupported" }, 400),
      );
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    await expect(api.listDir("/a.zip!/b.zip")).rejects.toThrow(
      "nested zips are unsupported",
    );
    await expect(api.listDir("/a.zip!/b.zip")).rejects.toBeInstanceOf(
      HttpError,
    );
  });
  ...
  it("overrides asks the route for one path, encoded, and throws like its siblings", async () => {
    ...
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ name: "Pack 03", credits }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
  ...
  it("getThumb names the unoccluded render, and only then", async () => {
    ...
    const fetchFn = vi.fn(() =>
      Promise.resolve(jsonResponse({ status: "miss" })),
    );
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
  ...
  it("getThumb carries the generation the server reported", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ status: "miss", gen: 99 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
  ...
  it("putThumb reports the generation its write landed under", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ ok: true, gen: 1234 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    expect(await api.putThumb({ path: "/m.stl", mtime: 42 })).toEqual({
      gen: 1234,
    });
  });
  ...
  it("putThumb drops a render the browser encoded as something else", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ ok: true, gen: 7 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
  ...
  it("putThumb keeps the orientation when it drops the render", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ ok: true, gen: 9 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    const png = new Blob([new Uint8Array([1, 2, 3])], { type: "image/png" });
    expect(
      await api.putThumb({
  ...
  it("putThumb reports nothing dropped on an ordinary write", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ ok: true, gen: 11 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
  ...
  it("putThumb passes a discard through even when the render is dropped", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ ok: true, gen: 10 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    const png = new Blob([new Uint8Array([1, 2, 3])], { type: "image/png" });
    await api.putThumb({
      path: "/m.stl",
  ...
  it("putThumb sends a render the browser encoded as WebP", async () => {
    ...
    const webp = new Blob([new Uint8Array([1, 2, 3])], { type: "image/webp" });
    expect(
      await api.putThumb({ path: "/m.stl", mtime: 42, png: webp }),
    ).toEqual({ gen: 7 });
  ...
  it("putThumb still sends the deletion a null carries", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(jsonResponse({ ok: true, gen: 8 }));
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    expect(
      await api.putThumb({ path: "/m.stl", mtime: 42, png: null }),
    ).toEqual({ gen: 8 });
    const [, init] = fetchFn.mock.calls[0] as [string, RequestInit];
  ...
  it("putThumb sends a deletion as null, and an absent png not at all", async () => {
    ...
    expect(
      Object.hasOwn(JSON.parse(plain.body as string) as object, "png"),
    ).toBe(false);
  });
  ...
  it("putThumb sends the generation it is conditional on, and only when it has one", async () => {
    ...
    expect(
      (JSON.parse(conditional.body as string) as Record<string, unknown>).ifGen,
    ).toBe(7);
    ...
    const [, plain] = fetchFn.mock.calls[0] as [string, RequestInit];
    expect(
      Object.hasOwn(JSON.parse(plain.body as string) as object, "ifGen"),
    ).toBe(false);
  });
  ...
  it("putThumb surfaces a refusal as a 412 HttpError", async () => {
    const fetchFn = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ error: "generation moved", gen: 99 }, 412),
      );
    const api = new HttpApiClient(fetchFn as unknown as typeof fetch);
    const err = await api
  ...
  it("carries the field a Refused body names, and nothing where a body names none", async () => {
    const refusing = vi
      ...
      .mockResolvedValue(
        jsonResponse(
          { error: "thumbnail writes are not offered", refused: "thumbWrites" },
          403,
        ),
      );
    ...
    const failing = vi
      .fn()
      .mockResolvedValue(jsonResponse({ error: "disk is full" }, 500));
  ...
  it("models asks for every model beneath one path, escaped", async () => {
    ...
    expect(fetchFn).toHaveBeenCalledWith(
      `/api/models?path=${encodeURIComponent("/kit")}`,
    );
  });
});
...
describe("withLocalFramings", () => {
  ...
  it("keeps an orbit release in this browser, sends nothing, and reports the pixels as not stored", async () => {
    ...
    expect(
      JSON.parse(store.raw.get("mb:framing:lib-a:/m.stl") as string),
    ).toEqual({
      camera: CAM,
      axis: "-z",
    });
  ...
  it("prefers this browser's framing over the one the server holds", async () => {
    ...
    const fetchFn = vi.fn(() =>
      Promise.resolve(
        jsonResponse({ status: "miss", camera: server, axis: "y", gen: 4 }),
      ),
    );
    const api = withLocalFramings(
      new HttpApiClient(fetchFn as unknown as typeof fetch),
  ...
  it("a discard deletes the local half, and the next read shows the server's", async () => {
    ...
    const fetchFn = vi.fn(() =>
      Promise.resolve(
        jsonResponse({ status: "miss", camera: server, axis: "y" }),
      ),
    );
    const api = withLocalFramings(
      new HttpApiClient(fetchFn as unknown as typeof fetch),
  ...
  it("rethrows a refusal of some other capability, keeping nothing", async () => {
    const fetchFn = vi.fn(() =>
      Promise.resolve(
        jsonResponse(
          { error: "the launcher is not offered", refused: "appLaunch" },
          403,
        ),
      ),
    );
  ...
  it("reads a hand-edited or malformed record as nothing stored", () => {
    ...
    store.raw.set(
      "mb:framing:lib-a:/m.stl",
      JSON.stringify({ camera: { az: "left" }, axis: "w" }),
    );
    expect(readLocalFraming("/m.stl", store, LIB)).toBeUndefined();
  });
});
