...
describe("isCurrentRender", () => {
  it("a hit at this build’s labels is current", () => {
    expect(isCurrentRender(current(), undefined, undefined, undefined)).toBe(
      true,
    );
  });
  ...
  it("a render from another rig is not current", () => {
    ...
    expect(
      isCurrentRender(
        { state: "hit", lighting: THUMB_LIGHTING },
        undefined,
        undefined,
        undefined,
      ),
  ...
  it("an unowned entry behind the pose recipe is stale", () => {
    ...
    expect(
      isCurrentRender(
        current({ posed: POSE_VERSION, poseKey }),
        undefined,
        undefined,
        POSE,
      ),
