...
function fakeHistory(): {
  ...
  const go = vi
    ...
    .mockImplementation((delta) => rewind(-(delta ?? 0)));
...
describe("leaving a similarity view", () => {
  ...
  it("one opened from a link clears to the listing, because there is nothing behind it", async () => {
    ...
    await mountAppAtCurrentUrl(
      `/?path=/models&similar=${encodeURIComponent(HERO)}`,
      NESTED,
    );
    await settle();
    expect(labels()).toEqual(["base.stl", "wing.stl"]);
  ...
  it("chained find-similars leave together, in one press", async () => {
    ...
    similar.mockResolvedValue({
      ...NEIGHBOURS,
      entries: [model("Kits/Baal/hero.stl")],
    });
    await findSimilarOn("base.stl");
    expect(location.search).toContain(
      encodeURIComponent("/models/Kits/Baal/base.stl"),
    );
    expect(history.state).toMatchObject({ similar: true, depth: 2 });
