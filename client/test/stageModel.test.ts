...
describe("stageModel", () => {
  it("centers staged bounds at the origin whatever the raw geometry offset", () => {
    for (const offset of OFFSETS) {
      ...
      expect(bounds.box.getCenter(new THREE.Vector3()).length()).toBeLessThan(
        1e-9,
      );
      // The pivot carries the whole offset; the model keeps its own transform.
      expect(pivot.position.distanceTo(offset.clone().negate())).toBeLessThan(
        1e-9,
      );
  ...
  it("reports the size the raw geometry had (centering rescales nothing)", () => {
    const staged = stageModel(
      makeScene(),
      makeMesh(new THREE.Vector3(9, 9, 9)),
      "z",
    );
    expect(staged.bounds.box.getSize(new THREE.Vector3()).toArray()).toEqual([
      2, 4, 6,
    ]);
...
function stageRadius(radius: number): {
  lit: LitScene;
...
describe("key-light shadow fit", () => {
  ...
  it("scales frustum, depth range and bias with the model — freezing the tuned multiples", () => {
    ...
    expect([cam.left, cam.right, cam.bottom, cam.top]).toEqual([
      -2 * r,
      2 * r,
      -2 * r,
      2 * r,
    ...
    expect(big.key.shadow.normalBias / small.key.shadow.normalBias).toBeCloseTo(
      100,
      4,
    );
    expect(big.key.position.length() / small.key.position.length()).toBeCloseTo(
      100,
      4,
    );
  });
  ...
  it("moves the key along its own direction, so the shading is untouched", () => {
    const tuned = (
      makeScene().rig.getObjectByName(KEY_LIGHT) as THREE.DirectionalLight
    ).position
      .clone()
      .normalize();
    for (const radius of [1, 100]) {
      const { key } = stageRadius(radius);
      expect(key.position.clone().normalize().distanceTo(tuned)).toBeLessThan(
        1e-9,
      );
    }
  });
...
function floorNormal(floor: THREE.Mesh): THREE.Vector3 {
  return new THREE.Vector3(0, 0, 1).applyQuaternion(
    floor.getWorldQuaternion(new THREE.Quaternion()),
  );
}
...
function sessionFloor(
  ...
  if (floor === undefined)
    throw new Error("session scene has no contact floor");
  return floor;
}
...
describe("contact floor", () => {
  it("rests on the face the spindle points away from, for every spindle", () => {
    for (const axis of AXES) {
      ...
      const expected = s
        .clone()
        .multiplyScalar(-half - FLOOR_SINK_R * bounds.radius);
      expect(
        floor.getWorldPosition(new THREE.Vector3()).distanceTo(expected),
      ).toBeLessThan(1e-6);
  ...
  it("snaps to the new spindle the moment the session changes axis", () => {
    ...
    const sink =
      FLOOR_SINK_R * Math.sqrt(HALF.x ** 2 + HALF.y ** 2 + HALF.z ** 2);
    expect(
      floor.position.distanceTo(new THREE.Vector3(0, -HALF.y - sink, 0)),
    ).toBeLessThan(1e-6);
  ...
  it("disposes its geometry and material when the session closes", () => {
    const session = new ViewerSession(
      makeMesh(new THREE.Vector3(1, 2, 3)),
      "y",
    );
    const floor = sessionFloor(session);
