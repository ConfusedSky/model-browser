...
describe("meaning search", () => {
  ...
  it("a phrase returns models whose names never mention it", async () => {
    ...
    expect(listDir).not.toHaveBeenCalledWith(
      "/models",
      expect.objectContaining({ q: "a winged demon" }),
    );
  ...
  it("flipping the mode re-runs the same text against the other corpus", async () => {
    ...
    listDir.mockImplementation(() =>
      Promise.resolve({ path: "/models", entries: [] }),
    );
  ...
  it("a name link is a name search even where the reader prefers meaning", async () => {
    ...
    await mountAppAtCurrentUrl(
      "/?path=/models&flat=1&q=widget&mode=name",
      NESTED,
    );
    await settle();
  ...
  it("the panel is never empty: meaning mode with no index still explains itself", async () => {
    ...
    await mountAppAtCurrentUrl(
      "/?path=/models&flat=1&q=demon&mode=meaning",
      NESTED,
    );
    await settle();
    await openSearchTab();
    ...
    expect(
      panel.querySelector('button[aria-label="Match folder names"]'),
    ).not.toBeNull();
  });
  ...
  it("says what the index covers when it covers nothing here, and names no path", async () => {
    ...
    expect(panel.textContent).toContain(
      "the index covers a location outside the library",
    );
  ...
  it("inside an archive it blames archives, not the folder", async () => {
    ...
    const panel = container.querySelector("aside")!;
    expect(panel.textContent).toContain(
      "does not cover the inside of archives",
    );
    expect(panel.textContent).not.toContain("not running");
  });
  ...
  it("tiles render at the index’s pose when nothing is cached", async () => {
    ...
    const [, camera, axis] = call! as unknown as [
      unknown,
      { az: number; el: number },
      string,
    ];
  ...
  it("a thumbnail the user already aimed survives a preference change too", async () => {
    ...
    expect(getThumb.mock.calls.at(-1)).toEqual([
  ...
  it("a tuning query scheduled for a view the user has left never fires", async () => {
    ...
    const top = container.querySelector<HTMLInputElement>(
  ...
  it("changing a parameter re-runs the committed query under it, and sticks", async () => {
    ...
    expect(
      JSON.parse(localStorage.getItem("model-browser:search-tuning")!).pool,
    ).toBe("max");
  });
  ...
  it("a typed bound reaches the URL, once, when the typing stops", async () => {
    ...
    expect(new URLSearchParams(location.search).get("min")).toBe(
      String(TUNING_DEFAULTS.minScore),
    );
  ...
  it("Back returns the bounds the entry was written under", async () => {
    ...
    expect(semanticSearch.mock.calls.at(-1)?.[2]).toMatchObject({
  ...
  it("offers the reset exactly when a bound or a parameter is off its default", async () => {
    ...
    const resetLink = () =>
      Array.from(
        container.querySelectorAll<HTMLButtonElement>("aside button"),
      ).find((b) => b.textContent?.trim().startsWith("Reset tuning"));
  ...
  it("a tuned link reproduces the sender’s parameters, not the reader’s", async () => {
    ...
    semanticSearch.mockResolvedValue(MEANING);
    await mountAppAtCurrentUrl(
      "/?path=/models&flat=1&q=demon&mode=meaning&score-raw=1",
      NESTED,
    );
    await settle();
  ...
  it("a link opened without the index keeps naming the meaning search", async () => {
    ...
    indexAvailability.mockResolvedValue({ state: "absent" });
    await mountAppAtCurrentUrl(
      "/?path=/models&flat=1&q=a+winged+demon&mode=meaning",
      NESTED,
    );
    await settle();
  ...
  it("offers the name search rather than performing it", async () => {
    indexAvailability.mockResolvedValue({ state: "absent" });
    await mountAppAtCurrentUrl(
      "/?path=/models&flat=1&q=widget&mode=meaning",
      NESTED,
    );
    await settle();
    listDir.mockClear();
    ...
    ).find((b) => b.textContent?.includes("Search names instead"))!;
    await click(escape);
    await settle();
