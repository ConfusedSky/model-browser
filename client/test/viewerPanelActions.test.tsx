...
const ALPHA: DirListing = {
  path: "/models/Alpha",
  entries: [model("Alpha/found.stl")],
};
const NEIGHBOURS = {
  path: "/models",
...
const action = (id: string): HTMLButtonElement =>
  actionRow()!.querySelector<HTMLButtonElement>(
...
describe("reset framing from the panel", () => {
   ...
   */
  async function openStoredModel(
    beforeMount: () => void = () => {},
  ): Promise<void> {
    await unmountApp();
    indexAvailability.mockResolvedValue({
    ...
    await mountAppAtCurrentUrl(
      "/?path=%2Fmodels&model=%2Fmodels%2FAlpha%2Ffound.stl",
      NESTED,
    );
    listDir.mockResolvedValue(NESTED);
    await wait(200);
  ...
  it("discards the stored orientation now, and the pixels follow after the close — two PUTs, no camera", async () => {
    ...
    const snapshot = renderThumbnail.mock.calls[0] as unknown as [
      unknown,
      typeof STORED,
      string,
    ];
  ...
  it("an orbit made after the reset is the last word — the queued pixels keep it", async () => {
    ...
    expect(
      writes
        .slice(writes.indexOf(orbited[0]!) + 1)
    ...
    const shot = renderThumbnail.mock.calls.at(-1) as unknown as [
      unknown,
      typeof STORED,
      string,
    ];
    expect(shot[1].distR).toBeCloseTo(
      (orbited[0]!.camera as typeof STORED).distR,
    );
  });
  ...
  it("discards an orbit made before it, so the close cannot write that orbit back", async () => {
    ...
    expect(writesFor(FOUND).filter((b) => b.camera === null).length).toBe(1);
    expect(
      writesFor(FOUND).filter(
        (b) => b.camera !== undefined && b.camera !== null,
      ),
    ).toEqual([]);
  });
  ...
  it("the queued re-render is pinned to what it read: an orbit landing first wins, and the render is skipped", async () => {
    ...
    putThumb.mockImplementation(
      (body: { camera?: unknown; ifGen?: number }) => {
        ...
        if (body.camera !== undefined && body.camera !== null)
          stored = { camera: body.camera, axis: "z" };
        gen++;
    ...
    const pinned = writesFor(FOUND).filter(
      (b) => b.png !== undefined && b.camera === undefined,
    );
  ...
  it("discards a framing given up while the mesh was still loading", async () => {
    ...
    for (const w of written) {
      expect((w.camera as typeof STORED).distR).toBeCloseTo(
        DEFAULT_CAMERA.distR,
      );
    }
  });
  ...
  it("re-frames to the pose the discard installed when the saved read lands after the press, and the pixels record it", async () => {
    ...
    expect(
      writesFor(FOUND).filter((b) => b.camera === null && b.axis === null)
        .length,
    ).toBe(1);
    ...
    expect(pixels[0]!.camera).toBeUndefined();
    const shot = renderThumbnail.mock.calls[0] as unknown as [
      unknown,
      typeof STORED,
      string,
    ];
  ...
  function harness(
    ...
    return {
      ...
      host: {
        ...
        queue: {
          push: (job: () => Promise<void>) => queued.push(job),
          whenResumed: () => Promise.resolve(),
        },
        setThumb,
