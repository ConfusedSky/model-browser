...
const NO_MATCHES_TRUNCATED: DirListing = {
  path: "/models",
  entries: [],
  truncated: true,
};
...
describe("file name search", () => {
  ...
  it("deep search submits q and renders relative-path results with the truncation notice", async () => {
    listDir.mockImplementation(
      (target: string, opts?: { flat?: boolean; q?: string }) =>
        Promise.resolve(
          target === "/models" && opts?.flat === true && opts?.q === "found"
            ? SEARCH_RESULT
            : NESTED,
        ),
    );
  ...
  it("a search submitted mid-navigation targets the in-flight destination", async () => {
    ...
    listDir.mockImplementation(
      (target: string, opts?: { flat?: boolean; q?: string }) => {
        if (target === "/models/Alpha" && opts?.flat !== true)
          return pendingNav;
        if (
          target === "/models/Alpha" &&
          opts?.flat === true &&
          ...
          return Promise.resolve({
            path: "/models/Alpha",
            entries: [model("found.stl")],
          });
        return Promise.reject(
          new Error(`unexpected listDir(${target}, ${JSON.stringify(opts)})`),
        );
      },
    );
  ...
  it("the truncation notice counts containers too, so folders-only truncation is not a claim about models", async () => {
    ...
    expect(container.textContent).toContain(
      "Showing 1 models and 1 folders; some entries were omitted.",
    );
  });
  ...
  it("a deep search with no matches says so, distinct from a filter hiding everything", async () => {
    listDir.mockImplementation(
      (target: string, opts?: { flat?: boolean; q?: string }) =>
        Promise.resolve(
          target === "/models" && opts?.flat === true && opts?.q === "zzz"
            ? NO_MATCHES
            : NESTED,
        ),
    );
  ...
  it("a failed search leaves no results label over the listing it never replaced", async () => {
    listDir.mockImplementation((_target: string, opts?: { q?: string }) =>
      opts?.q === undefined
        ...
        : Promise.reject(new Error("walk exploded")),
    );
  ...
  it("overlapping failed searches revert the label to the search that actually landed", async () => {
    ...
    listDir.mockImplementation((_target: string, opts?: { q?: string }) => {
      if (opts?.q === "found") return Promise.resolve(SEARCH_RESULT);
      if (opts?.q === "bee")
        return new Promise<DirListing>((_res, rej) => (failB = rej));
      if (opts?.q === "cee")
        return new Promise<DirListing>((_res, rej) => (failC = rej));
  ...
  it("the control survives a listing that outlives the reveal delay", async () => {
    ...
    listDir.mockImplementationOnce(
      () => new Promise<DirListing>((r) => (release = r)),
    );
