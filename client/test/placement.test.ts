...
describe("resolvePlacement (D4 chain)", () => {
  ...
  it("entry: no remembered placement is the top", () => {
    expect(
      resolvePlacement({ kind: "entry", placement: null }, listing),
    ).toEqual({ kind: "top" });
  });
  ...
  it("up: the parent anchor wins over the child when both are present", () => {
    ...
    expect(
      resolvePlacement({ kind: "up", placement, child: "/Kit A" }, listing),
    ).toEqual({
      kind: "anchor",
      path: "/Kit C/part.stl",
      offset: -5,
  ...
  it("up from a deep arrival: no anchor, the child folder is centred", () => {
    expect(
      resolvePlacement(
        { kind: "up", placement: null, child: "/Kit A" },
        listing,
      ),
    ).toEqual({
      kind: "center",
  ...
  it("up: an absent anchor falls to the child when the child is present", () => {
    ...
    expect(
      resolvePlacement({ kind: "up", placement, child: "/Kit B" }, listing),
    ).toEqual({
      kind: "center",
      path: "/Kit B",
    });
  ...
  it("up in flat: a folder anchor and a folder child, neither shown, fall to the top", () => {
    ...
    expect(
      resolvePlacement({ kind: "up", placement, child: "/Kit B" }, flat),
    ).toEqual({
      kind: "top",
    });
  });
  ...
  it("reveal: a present path is centred, an absent one is the top", () => {
    expect(
      resolvePlacement({ kind: "reveal", path: "/Kit B" }, listing),
    ).toEqual({
      kind: "center",
      path: "/Kit B",
    });
    expect(
      resolvePlacement({ kind: "reveal", path: "/Gone" }, listing),
    ).toEqual({ kind: "top" });
  });
});

describe("measurePlacement (D1)", () => {
  const tile = (path: string, top: number, height = 100) => ({
    path,
    top,
    bottom: top + height,
  });
...
describe("applyIn", () => {
  it("anchor: scrolls so the tile top sits at the remembered offset", () => {
    ...
    expect(
      applyIn(f.scroller, { kind: "anchor", path: "/b", offset: -20 }),
    ).toBe(true);
  ...
  it("a tile not in the grid is false and the scroller is untouched", () => {
    ...
    expect(
      applyIn(f.scroller, { kind: "anchor", path: "/zzz", offset: -1 }),
    ).toBe(false);
  ...
  it("a scroller that cannot be measured is false, never a throw", () => {
    ...
    expect(applyIn(f.scroller, { kind: "anchor", path: "/a", offset: 0 })).toBe(
      false,
    );
