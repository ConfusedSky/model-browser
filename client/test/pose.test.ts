...
function rotationToZUp(up: [number, number, number]): THREE.Matrix4 {
  ...
  return new THREE.Matrix4().makeRotationAxis(
    axis,
    Math.acos(Math.min(1, Math.max(-1, v.dot(z)))),
  );
}
...
function azimuthZero(up: [number, number, number]): [number, number, number] {
  const o = new THREE.Vector3(1, 0, 0).applyMatrix4(
    rotationToZUp(up).clone().transpose(),
  );
  return [o.x, o.y, o.z];
}

/** Where the index's camera sits, in *file* space, for one of its views. */
function indexCameraDirection(
...
describe("index pose → camera", () => {
  ...
  it("an up axis outside the six is a fault, not a rounding", () => {
    ...
    expect(
      cameraForPose(pose([0.02, 0.999, 0], 0, 0), DEFAULT_CAMERA),
    ).toBeNull();
  });
  ...
  it("an azimuth_zero not perpendicular to up is malformed, not projected", () => {
    const p = {
      ...pose(UP.y, 0, 0),
      azimuth_zero: [0, 1, 0] as [number, number, number],
    };
  ...
  it("derives az as the front azimuth plus atan2(a·u₀, b·u₀) in the file frame, about axisOfTriple(up)", () => {
    ...
    for (const [name, up] of Object.entries(UP) as [
      OrbitAxis,
      ...
      for (const u0 of [
        a,
        b,
        [-a[0], -a[1], -a[2]] as const,
        [-b[0], -b[1], -b[2]] as const,
      ]) {
        const p: IndexPose = {
          ...pose(up, 30, 20),
          azimuth_zero: [u0[0], u0[1], u0[2]],
        };
        ...
        expect(out.camera.az, `${name} u0=${u0.join(",")}`).toBeCloseTo(
          (30 * Math.PI) / 180 + offset,
          9,
        );
  ...
  it("keeps the model upright: camera up is the model up, in file space", () => {
    ...
    for (const [name, up] of Object.entries(UP) as [
      ...
      expect(spindle.distanceTo(new THREE.Vector3(...up)), name).toBeLessThan(
        1e-6,
      );
    }
  });
