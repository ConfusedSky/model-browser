...
describe("a similarity view", () => {
  ...
  it("shows the model the neighbours were computed from, first and marked as the subject", async () => {
    ...
    expect(neighbour!.getAttribute("aria-label")).not.toContain(
      "compared against",
    );
  });
  ...
  it("announces a neighbour’s numbers with the scale spelled out", async () => {
    ...
    expect(subject!.getAttribute("aria-label")).not.toContain(
      "similarity 0.912",
    );
  });
  ...
  it("a model the index has not embedded is explained as not indexed yet — without a re-probe", async () => {
    indexAvailability.mockResolvedValue(READY);
    similar.mockRejectedValue(
      await httpError(404, "/models/Kits/Baal/hero.stl is not in the cache"),
    );
    await mountAppAtCurrentUrl(LINK, NESTED);
    await settle();
  ...
  it("states the same fact without the instruction where the host is not the viewer’s", async () => {
    ...
    expect(container.textContent).toContain(
      "is not in the index, so it has no neighbours yet",
    );
  ...
  it("a model inside an archive is a different sentence, and costs no request", async () => {
    ...
    await mountAppAtCurrentUrl(
      `/?path=/models&similar=${encodeURIComponent(inZip)}`,
      NESTED,
    );
    await settle();
    ...
    expect(similar).not.toHaveBeenCalled();
    expect(container.textContent).toContain(
      "inside an archive are outside what the index covers",
    );
