...
const like = (
  model: string,
  over: { k?: number; pool?: Tuning["pool"] } = {},
): Subject => ({
  kind: "similar",
  model,
  k: over.k ?? SIMILAR_K,
...
): SearchState => initialState(view(over), index);
...
const urlOf = (state: SearchState): string =>
  serializeView(toUrlView(state.view));
...
describe("the reducer, finding by finding", () => {
  ...
  it("the deferral fires only for the view that made it", () => {
    let s = run(start({ flat: true }, WARMING), {
      type: "setMode",
      mode: "meaning",
    });
  ...
  it("tuning survives the restore compare, and the restore", () => {
    const tuned: View = view({
      subject: asks("dragon"),
      mode: "meaning",
      flat: true,
    });
    let s = land(reducer(start({}, READY), { type: "restore", view: tuned }), {
      entries: [],
    });
  ...
  it("a count beneath a floor is the question too, now that they compose", () => {
    ...
    const s = land(
      reducer(start({}, READY), { type: "restore", view: meaning }),
      {
        entries: [entry("a.stl")],
      },
    );
    ...
    expect(pendingRequest(recounted)).toMatchObject({
      kind: "meaning",
      tuning: { top: 12 },
    });
    ...
    const counted: View = {
      ...s.view,
      tuning: { ...s.view.tuning, minScore: undefined },
    };
    const s2 = land(reducer(s, { type: "restore", view: counted }), {
      entries: [],
    });
    ...
    expect(pendingRequest(reasked)).toMatchObject({
      kind: "meaning",
      tuning: { top: 12 },
    });
    ...
    const floorOnly: View = {
      ...s.view,
      tuning: { ...s.view.tuning, top: undefined },
    };
    const s3 = land(reducer(s, { type: "restore", view: floorOnly }), {
      entries: [],
    });
    const patched = reducer(s3, {
      type: "restore",
      view: { ...floorOnly, model: null },
    });
  ...
  it("the flat toggle is a question only when nothing is committed", () => {
    ...
    const searching = land(search(start({ flat: false }), "dragon"), {
      entries: [],
    });
    ...
    const listing = land(
      reducer(start({ flat: false }), {
        type: "restore",
        view: view({ flat: false }),
      }),
      { entries: [] },
    );
    const deepened = reducer(listing, {
      type: "restore",
      view: { ...listing.view, flat: true },
    });
    expect(pendingRequest(deepened)).toMatchObject({
      kind: "listing",
      flat: true,
      q: null,
    });
  ...
  it("a stale index is impossible: the corpus decision reads it from state", () => {
    ...
    s = reducer(land(s, { entries: [entry("a.stl")] }), {
      type: "index",
      availability: READY,
    });
    expect(pendingRequest(s)).toMatchObject({
  ...
  it("every URL-owning transition names the whole view — its own mode’s half of it", () => {
    ...
    for (const mode of ["name", "meaning"] as const) {
      ...
      let s = land(reducer(start({}, READY), { type: "restore", view: full }), {
        entries: [],
      });
  ...
  it("the residue dies with its result", () => {
    ...
    expect(labelInputs(s)).toMatchObject({
      meaning: true,
      weak: true,
      capped: true,
    });
  ...
  it("a mode flip while the index is not ready defers instead of substituting", () => {
    let s = land(search(start({}, WARMING), "dragon"), {
      entries: [entry("dragon.stl")],
    });
    expect(s.view.mode).toBe("name");
  ...
  it("navigating re-seeds all four options from the preferences on the action", () => {
    ...
    let s = land(reducer(start({}, READY), { type: "restore", view: link }), {
      entries: [],
    });
    ...
    expect(s.inflight?.view).toMatchObject({
      ...own,
      path: "/other",
      subject: { kind: "none" },
    });
...
describe("the view has a subject", () => {
  it("a similarity subject and a query subject replace each other", () => {
    let s = land(search(start({}, READY), "dragon"), {
      entries: [entry("a.stl")],
    });
  ...
  it("a similarity deep link waits, stands in nested, and fires under its own provenance", () => {
    const link: View = view({
      path: "/lib/sub",
      subject: like("/lib/a.stl"),
      flat: true,
    });
    let s = reducer(start({ path: "/lib/sub" }, null), {
      type: "restore",
      view: link,
    });
  ...
  it("entering a similarity view empties the draft, and erasing text under one still dismisses", () => {
    ...
    s = land(reducer(s, { type: "similar", model: "/lib/a.stl" }), {
      entries: [entry("b.stl")],
    });
    expect(s.drafts.queryText).toBe("");
    expect(s.view.subject).toEqual(like("/lib/a.stl"));
  ...
  it("a similarity URL names the model, the place and the toggle — and nothing else", () => {
    ...
    const s = land(reducer(start({}, READY), { type: "restore", view: full }), {
      entries: [],
    });
    const url = urlOf(s);
  ...
  it("an empty similarity result is an empty answer, not an empty folder", () => {
    ...
    expect(labelInputs(empty)).toMatchObject({
      meaning: false,
      weak: false,
      capped: false,
    });
    ...
    const listing = land(
      reducer(start({}, READY), { type: "restore", view: view() }),
      {
        entries: [],
      },
    );
  ...
  it("a different parameter is a different question: Back across one re-asks", () => {
    ...
    const pooled: View = {
      ...at40,
      subject: like("/lib/a.stl", { k: 40, pool: "mean" }),
    };
  ...
  it("the URL carries the two parameters a similarity subject reads, and only off their defaults", () => {
    ...
    const tuned = land(
      reducer(start({}, READY), {
        type: "restore",
        view: view({
          subject: like("/lib/a.stl", { k: 40, pool: "max" }),
          kinds: "models",
        }),
      }),
      { entries: [] },
...
describe("acceptance: one rule for every answer", () => {
  ...
  it("the pose wave is kept only for the landing that fired it", () => {
    ...
    const landed = land(
      reducer(start(), { type: "navigate", path: "/lib", prefs: PREFS }),
      {
        entries: [entry("a.stl")],
      },
    );
    ...
    expect(
      reducer(waved, { type: "listingPoses", id: id + 1, poses: {} }),
    ).toBe(waved);
    expect(reducer(landed, { type: "listingPoses", id: id - 1, poses })).toBe(
      landed,
    );
    // Nor before anything has landed.
    expect(
      reducer(start(), { type: "listingPoses", id: 1, poses }).listingPoses,
    ).toBeNull();
  });
  ...
  it("a landing drops the poses the last one was given, and a patch keeps them", () => {
    ...
    expect(
      reducer(s, { type: "modelOpen", path: "/lib/a.stl" }).listingPoses,
    ).toBe(poses);
    expect(reducer(s, { type: "setKinds", kinds: "models" }).listingPoses).toBe(
      poses,
    );

    const elsewhere = land(
      reducer(s, { type: "navigate", path: "/lib/kit", prefs: PREFS }),
      {
        entries: [entry("a.stl")],
      },
    );
